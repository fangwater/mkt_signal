...
2. 在 `tokio::LocalSet` 上启动订阅任务：
   - pre-trade resample：`signal_pubs/pre_trade_positions|pre_trade_exposure|pre_trade_risk`，收到后直接广播给 WebSocket 客户端。
...
## 数据模型要点
- `pre_trade_positions` / `pre_trade_exposure` / `pre_trade_risk`：`signal_pubs/` 的重采样消息，`viz_server` 直接转发。
