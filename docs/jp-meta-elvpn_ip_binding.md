...
最后更新: 2026-08-16。**分配、改写 `trade_engine.toml local_ips`、新开或下线任何用独立 source IP 的环境时，请同步更新本文件。**
...
`ens42`（第二块网卡，metric 200）上的 `172.31.46.90/20`–`.93/20` 已绑定，**尚无** `trade_engine.toml` 引用。不要在未更新本表前占用。
...
| `172.31.35.231` | `52.69.78.134` | 已使用 / 固定 | 资金费率。`binance_fr_arb01`–`04`、`gate_fr_arb01`、`bitget_fr_arb02`、`okex_fr_arb01` |
...
| 私网 IP | 公网 IP | 状态 | 当前用途 |
| --- | --- | --- | --- |
| `172.31.46.90` | `52.69.209.108` | 已绑定 / 未分配 | 无策略引用 |
| `172.31.46.91` | `54.199.82.56` | 已绑定 / 未分配 | 无策略引用 |
| `172.31.46.92` | `52.192.54.88` | 已绑定 / 未分配 | 无策略引用 |
| `172.31.46.93` | `18.181.48.65` | 已绑定 / 未分配 | 无策略引用 |
