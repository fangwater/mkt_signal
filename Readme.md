...
首先实现这个功能。
2、当收到mm hedge signal的时候，打印所有当前的MarketMakerhedgestargey 累积头寸的正负（即多空累积成交了多少），然后，strategy需要一个关键的功能。仿照目前的query，需要新增一个query msgs。
msg的内容包括symbol， 和期间累积成交， b/s方向各成交了多少量。仿照目前pre-trade的query hedge msg。

query消息会返回一个distrute。即一个挂单范围。目前的xarb的返回什么？
