...
fn hyperliquid_coin_from_internal(symbol: &str) -> String {
    ...
    if normalized.ends_with("USDC") && normalized.len() > 4 {
    ...
    pub async fn new(cfg: &Config) -> Self {
        ...
        for chunk in symbols.chunks(batch_size) {
            ...
            if let Some(ref ch) = trade_channel {
                trade_subscribe_msgs.push(construct_subscribe_message(&exchange, chunk, ch));
            }
            if let Some(ref ch) = kline_channel {
                kline_subscribe_msgs.push(construct_subscribe_message(&exchange, chunk, ch));
            }
            if let Some(ref ch) = best_price_spread_channel {
                ask_bid_spread_msgs.push(construct_subscribe_message(&exchange, chunk, ch));
            }
        }
        Self {
