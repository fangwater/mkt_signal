...
impl MktSignalApp {
    pub async fn new(config: &'static Config) -> Result<Self> {
        info!(
            "Initializing MktSignalApp for exchange: {}",
            config.get_exchange()
        );
    ...
    async fn initialize_and_start(&mut self) -> Result<()> {
        ...
        if self.proxy.is_none() {
            ...
            let Some(incremental_rx) = self.incremental_rx.take() else {
                anyhow::bail!("incremental_rx already taken")
            };
            let Some(trade_rx) = self.trade_rx.take() else {
                anyhow::bail!("trade_rx already taken")
            };
            let Some(kline_rx) = self.kline_rx.take() else {
                anyhow::bail!("kline_rx already taken")
            };
            let Some(derivatives_rx) = self.derivatives_rx.take() else {
                anyhow::bail!("derivatives_rx already taken")
            };
            let Some(signal_rx) = self.signal_rx.take() else {
                anyhow::bail!("signal_rx already taken")
            };
            let Some(ask_bid_spread_rx) = self.ask_bid_spread_rx.take() else {
                anyhow::bail!("ask_bid_spread_rx already taken")
            };
    ...
    async fn main_event_loop(&mut self) -> Result<()> {
        info!(
            "Primary restart scheduled at: {:?}",
            self.next_primary_restart
        );
        info!(
            "Secondary restart scheduled at: {:?}",
            self.next_secondary_restart
        );
        ...
        while !self.cancellation_token.is_cancelled() {
            let now = Instant::now();
            let time_to_primary = self
                .next_primary_restart
                .saturating_duration_since(now)
                .as_secs();
            let time_to_secondary = self
                .next_secondary_restart
                .saturating_duration_since(now)
                .as_secs();
    ...
    fn format_status_table_dual(&self, primary_restart: u64, secondary_restart: u64) -> String {
        format!(
            "\n\
             ...
             |-------------------------|-------------|",
            primary_restart, secondary_restart
        )
    }
}
