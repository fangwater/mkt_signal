...
pub struct LsegFormulaPlan {
    pub(crate) ordered_factors: Vec<LsegFactorBinding>,
}

impl LsegFormulaPlan {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct LsegFactorBinding {
    pub(crate) name: String,
    pub(crate) factor_id: Option<LsegFactorId>,
    pub(crate) extra_factor_id: Option<LsegExtraFactorId>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LsegExtraFactorId {
    AvgPrice,
    BuyAvgPrice,
    SellAvgPrice,
    SmallBuy,
...
fn build_factor_plan(scope: &str, factors: Vec<String>) -> Result<LsegFormulaPlan> {
    let mut ordered_factors = Vec::with_capacity(factors.len());
    for name in factors {
        let factor_id = LsegFactorId::from_name(&name);
        let extra_factor_id = LsegExtraFactorId::from_name(&name);
        if factor_id.is_none() && extra_factor_id.is_none() {
            bail!("factor-plan unmapped: {} {}", scope, name);
        }
        ordered_factors.push(LsegFactorBinding {
            name,
            factor_id,
            extra_factor_id,
        });
    ...
    Ok(LsegFormulaPlan { ordered_factors })
}

fn decode_symbol_factor_plans(payload: FactorPlanResp) -> Result<HashMap<String, LsegFormulaPlan>> {
...
pub(crate) async fn load_symbol_factor_plans_from_tlen_server(
    ...
    venue_slug: &str,
    config_type: &str,
) -> Result<HashMap<String, LsegFormulaPlan>> {
