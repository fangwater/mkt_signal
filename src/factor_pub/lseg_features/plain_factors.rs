...
use super::view::{F64SeriesView, LsegSeries, OptF64SeriesView};

use super::app::{
    depth_best_ask, depth_best_bid, depth_level_amount, depth_level_price, depth_sum_price,
    LsegDepthStats10, LsegFactorEngine, LSEG_DEPTH_LEVELS, LSEG_TOP5_LEVELS,
};
    ...
    ($binding:expr, $depth:expr, $series:expr, { $($variant:ident => $func:ident($arg:ident),)* }) => {
        match $binding.factor_id {
            $(Some(LsegFactorId::$variant) => {
                Some(LsegFactorEngine::wrap_factor_value($arg.and_then($func)))
            })*
            _ => None,
        }
    };
...
fn bid0_price_series(series: &LsegSeries<'_>) -> Vec<f64> {
    (0..series.mid_price.len())
        .map(|i| series.mid_price[i] - series.spread[i] / 2.0)
        .collect()
}

fn ask0_price_series(series: &LsegSeries<'_>) -> Vec<f64> {
    (0..series.mid_price.len())
        .map(|i| series.mid_price[i] + series.spread[i] / 2.0)
        .collect()
}

fn compute_factor_005(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.mid_price, 1)
}

fn compute_factor_007(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_factor_013(depth: &LsegDepthStats10) -> Option<f64> {
    ratio(
        depth.sum_bid_amount(LSEG_TOP5_LEVELS),
        depth.sum_bid_amount(LSEG_DEPTH_LEVELS),
    )
}

fn compute_factor_015(depth: &LsegDepthStats10) -> Option<f64> {
    let total_bid_price = depth_sum_price(&depth.bids, LSEG_DEPTH_LEVELS);
    finite_opt(Some(
        (0..LSEG_DEPTH_LEVELS)
...
fn compute_factor_034(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_factor_039(series: &LsegSeries<'_>) -> Option<f64> {
    compute_factor_007(series)
}

fn compute_factor_044(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_DEPTH_LEVELS)
            .map(|i| depth_level_price(&depth.asks, i) - depth_level_price(&depth.bids, i))
            .sum::<f64>()
            / LSEG_DEPTH_LEVELS as f64,
    ))
}

fn compute_factor_050(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_std_last(&series.mean_ask_price5, 30).ok().flatten()
}

fn compute_factor_071(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_TOP5_LEVELS)
            .map(|i| depth_level_amount(&depth.bids, i).sqrt())
            .sum::<f64>()
            / LSEG_TOP5_LEVELS as f64,
    ))
}

fn compute_factor_072(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_TOP5_LEVELS)
...
fn compute_factor_078(series: &LsegSeries<'_>) -> Option<f64> {
    let bid = current(&series.total_bid5)?;
    let total = bid + current(&series.total_ask5)?;
    ratio(bid, total)
}

fn compute_factor_081(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.outer_level_volume, 3)
        .ok()
        .flatten()
}

fn compute_factor_082(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_sum_last_from_series(&series.outer_level_volume, 3, 3)
        .ok()
        .flatten()
}

fn compute_factor_083(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
...
fn compute_factor_084(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
...
fn compute_factor_090(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_DEPTH_LEVELS)
            .map(|i| depth_level_price(&depth.asks, i) - depth_level_price(&depth.bids, i))
            .sum::<f64>()
...
fn compute_factor_092(depth: &LsegDepthStats10) -> Option<f64> {
    let bid: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.bids, i))
        .collect();
    let ask: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.asks, i))
        .collect();
    ratio(pop_std(&bid)?, pop_std(&ask)?)
}

fn compute_factor_098(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.bids, i))
        .collect();
    if vals.iter().any(|value| !value.is_finite()) {
...
fn compute_factor_099(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.asks, i))
        .collect();
    if vals.iter().any(|value| !value.is_finite()) {
...
fn compute_factor_100(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.bids, i))
        .collect();
...
fn compute_factor_101(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.asks, i))
        .collect();
    let mean = vals.iter().sum::<f64>() / vals.len() as f64;
...
fn compute_factor_104(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_TOP5_LEVELS)
            .map(|i| depth_level_price(&depth.asks, i) - depth_level_price(&depth.bids, i))
            .sum::<f64>()
...
fn compute_factor_105(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_DEPTH_LEVELS)
            .map(|i| depth_level_price(&depth.asks, i) - depth_level_price(&depth.bids, i))
            .sum::<f64>()
...
fn compute_factor_106(depth: &LsegDepthStats10) -> Option<f64> {
    let bids = (0..LSEG_DEPTH_LEVELS)
        .map(|i| finite_opt(Some(depth_level_price(&depth.bids, i))))
        .collect::<Option<Vec<_>>>()?;
    let asks = (0..LSEG_DEPTH_LEVELS)
...
fn compute_factor_109(depth: &LsegDepthStats10) -> Option<f64> {
...
fn compute_factor_112(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.outer_ask_price, 3, 3)?;
    finite_opt(Some(std * std))
}

fn compute_factor_117(depth: &LsegDepthStats10) -> Option<f64> {
    let mut num = 0.0;
    let mut den = 0.0;
...
fn compute_factor_127(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.factor_127_bid_price_kurt, 300)
        .ok()
        .flatten()
}

fn compute_factor_131(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.factor_131_ask_price_kurt, 200)
        .ok()
        .flatten()
}

fn compute_factor_132(series: &LsegSeries<'_>) -> Option<f64> {
    ratio(current(&series.total_ask5)?, current(&series.total_bid5)?)
}

fn compute_factor_135(series: &LsegSeries<'_>) -> Option<f64> {
    compute_factor_005(series)
}

fn compute_factor_136(depth: &LsegDepthStats10) -> Option<f64> {
    let ask_strength = (0..LSEG_TOP5_LEVELS)
        .map(|i| depth_level_price(&depth.asks, i) * depth_level_amount(&depth.asks, i))
        .sum::<f64>();
    let bid_strength = (0..LSEG_TOP5_LEVELS)
...
fn compute_factor_137(series: &LsegSeries<'_>) -> Option<f64> {
    compute_factor_007(series)
}

fn compute_factor_138(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
...
fn compute_factor_140(depth: &LsegDepthStats10) -> Option<f64> {
    let ratios: Vec<Option<f64>> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| {
...
fn compute_factor_141(series: &LsegSeries<'_>) -> Option<f64> {
    current(&series.total_bid5)
}

fn compute_factor_142(series: &LsegSeries<'_>) -> Option<f64> {
    current(&series.total_ask5)
}

fn compute_factor_143(depth: &LsegDepthStats10) -> Option<f64> {
    compute_factor_138(depth)
}

fn compute_factor_145(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_TOP5_LEVELS)
            .map(|i| depth_level_price(&depth.bids, i) * depth_level_amount(&depth.bids, i))
            .sum(),
...
fn compute_factor_146(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_TOP5_LEVELS)
            .map(|i| depth_level_price(&depth.asks, i) * depth_level_amount(&depth.asks, i))
            .sum(),
...
fn compute_factor_147(depth: &LsegDepthStats10) -> Option<f64> {
    (0..LSEG_DEPTH_LEVELS)
        .map(|i| {
            finite_opt(Some(
...
fn compute_factor_148(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_TOP5_LEVELS)
        .map(|i| depth_level_amount(&depth.bids, i) + depth_level_amount(&depth.asks, i))
        .collect();
    pop_std(&vals)
}

fn compute_factor_149(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (1..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_price(&depth.bids, i) - depth_level_price(&depth.asks, i))
        .collect();
    finite_opt(Some(vals.iter().sum::<f64>() / vals.len() as f64))
}

fn compute_factor_150(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_amount(&depth.bids, i) + depth_level_amount(&depth.asks, i))
        .collect();
...
fn compute_factor_153(depth: &LsegDepthStats10) -> Option<f64> {
    let bid: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| depth_level_amount(&depth.bids, i))
        .collect();
    let ask: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
...
fn compute_factor_154(depth: &LsegDepthStats10) -> Option<f64> {
    let vals: Vec<f64> = (0..LSEG_TOP5_LEVELS)
        .map(|i| depth_level_amount(&depth.bids, i) + depth_level_amount(&depth.asks, i))
        .collect();
...
fn compute_factor_155(depth: &LsegDepthStats10) -> Option<f64> {
    let ratios: Vec<f64> = (0..LSEG_DEPTH_LEVELS)
        .map(|i| {
            let b = depth_level_amount(&depth.bids, i);
            let a = depth_level_amount(&depth.asks, i);
            ratio(b, a + b)
...
fn compute_factor_157(series: &LsegSeries<'_>) -> Option<f64> {
    current_opt(&series.factor_157_bid_ask_diff_std)
}

fn compute_factor_158(depth: &LsegDepthStats10) -> Option<f64> {
    finite_opt(Some(
        (0..LSEG_DEPTH_LEVELS)
            .map(|i| {
...
fn compute_factor_161(series: &LsegSeries<'_>) -> Option<f64> {
    let bid0 = bid0_price_series(series);
    diff_last(bid0.as_slice(), 1)
}

fn compute_factor_162(series: &LsegSeries<'_>) -> Option<f64> {
    let ask0 = ask0_price_series(series);
    diff_last(ask0.as_slice(), 1)
}

fn compute_factor_163(series: &LsegSeries<'_>) -> Option<f64> {
    current(&series.relative_spread)
}

fn compute_factor_167(series: &LsegSeries<'_>) -> Option<f64> {
    finite_opt(Some(
        (current(&series.bid0v)? + current(&series.ask0v)?) / 2.0,
    ))
}

fn compute_factor_169(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_factor_171(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_factor_172(series: &LsegSeries<'_>) -> Option<f64> {
    let bid0 = bid0_price_series(series);
    sample_std_last(bid0.as_slice(), 10, 2)
}

fn compute_factor_173(series: &LsegSeries<'_>) -> Option<f64> {
    current(&series.spread)
}

fn compute_factor_174(depth: &LsegDepthStats10) -> Option<f64> {
    let (bid0p, bid0v) = depth_best_bid(depth);
    let (ask0p, ask0v) = depth_best_ask(depth);
    finite_opt(Some(ask0p * ask0v - bid0p * bid0v))
}

fn compute_factor_177(series: &LsegSeries<'_>) -> Option<f64> {
