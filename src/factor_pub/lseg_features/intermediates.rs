...
pub fn build_true_range_series(series: &LsegSeries<'_>) -> Option<Vec<Option<f64>>> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
pub fn build_ad_line_rolling360(series: &LsegSeries<'_>) -> Option<Vec<Option<f64>>> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
pub fn build_obv_cumulative(series: &LsegSeries<'_>) -> Option<Vec<f64>> {
...
pub fn compute_tp_vpi_006(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    if n < 373 {
        return None;
    }
