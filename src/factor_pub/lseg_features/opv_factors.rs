...
use super::view::{F64SeriesView, LsegSeries};

use super::app::LsegFactorEngine;
use super::factor_enum::LsegFactorId;
use super::plan::LsegFactorBinding;

macro_rules! dispatch_opv {
    ($binding:expr, $series:expr, { $($variant:ident => $func:ident,)* }) => {
        match $binding.factor_id {
            $(Some(LsegFactorId::$variant) => {
                Some(LsegFactorEngine::wrap_factor_value($series.and_then($func)))
            })*
            _ => None,
        }
    };
    ...
    (0..values.len())
        .map(|i| {
            if i < periods {
                ...
                let current = values.value_at(i);
                let previous = values.value_at(i - periods);
                if !current.is_finite() || !previous.is_finite() {
                    Some(f64::NAN)
                } else {
                    finite_opt(Some(current - previous))
                }
            }
        })
        .collect()
...
fn binary_opt_vec(
    ...
    (0..n)
        .map(|i| match (a[i], b[i]) {
            (Some(x), Some(y)) if !x.is_finite() || !y.is_finite() => Some(f64::NAN),
            (Some(x), Some(y)) => f(x, y),
            _ => None,
        })
        .collect()
...
fn unary_opt_vec(values: &[Option<f64>], f: impl Fn(f64) -> Option<f64>) -> Vec<Option<f64>> {
    values
        ...
        .map(|value| match value {
            Some(value) if !value.is_finite() => Some(f64::NAN),
            Some(value) => f(*value),
            None => None,
        })
        .collect()
...
fn sign_close_open(close: f64, open: f64) -> f64 {
    if !close.is_finite() || !open.is_finite() {
        f64::NAN
    } else if close > open {
        1.0
    } else if close < open {
...
fn clv_series(series: &LsegSeries<'_>) -> Vec<Option<f64>> {
    zip_map3_opt(
        &series.close,
        &series.low,
        &series.high,
        |close, low, high| {
            if !close.is_finite() || !low.is_finite() || !high.is_finite() {
                return Some(f64::NAN);
            }
            let den = high - low;
            if den.abs() <= 1e-12 {
...
fn typical_price_series(series: &LsegSeries<'_>) -> Vec<Option<f64>> {
    zip_map3_opt(
        &series.high,
        &series.low,
        &series.close,
        |high, low, close| {
            if !high.is_finite() || !low.is_finite() || !close.is_finite() {
                ...
                finite_opt(Some((high + low + close) / 3.0)).or(Some(f64::NAN))
            }
        },
    )
}

fn tr_series(series: &LsegSeries<'_>) -> Vec<Option<f64>> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn bp_series(series: &LsegSeries<'_>) -> Vec<Option<f64>> {
    ...
    for i in 0..n {
        ...
        let low = series.low.value_at(i);
        let prev_close = series.close.value_at(i - 1);
        if !close.is_finite() || !low.is_finite() || !prev_close.is_finite() {
            out.push(Some(f64::NAN));
            continue;
        }
...
fn plus_dm_series(series: &LsegSeries<'_>) -> Vec<f64> {
    let n = series.high.len().min(series.low.len());
    let mut out = Vec::with_capacity(n);
    out.push(0.0);
    for i in 1..n {
        let high = series.high.value_at(i);
        let previous_high = series.high.value_at(i - 1);
        let low = series.low.value_at(i);
        let previous_low = series.low.value_at(i - 1);
        if !high.is_finite()
        ...
        let high_diff = high - previous_high;
        let low_diff = low - previous_low;
        out.push(if high_diff > low_diff { high_diff } else { 0.0 });
    }
    out
}

fn minus_dm_series(series: &LsegSeries<'_>) -> Vec<f64> {
    let n = series.high.len().min(series.low.len());
    let mut out = Vec::with_capacity(n);
    out.push(0.0);
    for i in 1..n {
...
fn body_last(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    finite_opt(Some((close - open).abs()))
}

fn upper_shadow_last(series: &LsegSeries<'_>) -> Option<f64> {
...
fn lower_shadow_last(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_010(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_011(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_016(series: &LsegSeries<'_>) -> Option<f64> {
    let middle = rolling_mean_exact(&series.close, 20)?;
    let std = sample_std_last(&series.close, 20, 20)?;
    finite_opt(Some(middle - 2.0 * std))
}

fn compute_td_ti_017(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_018(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_019(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 20, 20)?;
    finite_opt(Some(4.0 * std))
}

fn compute_td_ti_020(series: &LsegSeries<'_>) -> Option<f64> {
    let typical = typical_price_series(series);
    let std = rolling_std_opt_last(&typical, 5, 5)?;
    finite_opt(Some(4.0 * std))
}

fn compute_td_ti_021(series: &LsegSeries<'_>) -> Option<f64> {
    let kama = recursive_kama_like_series(&series.close, 30);
    rolling_mean_exact(&kama, 30)
}

fn compute_td_ti_022(series: &LsegSeries<'_>) -> Option<f64> {
    let kama = recursive_kama_like_series(&series.amount, 14);
    rolling_mean_exact(&kama, 14)
}

fn compute_td_ti_023(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_024(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_025(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_026(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_td_ti_027(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
    ...
    for i in 0..n {
        if i > 0
            && (!series.close.value_at(i - 1).is_finite() || !series.close.value_at(i).is_finite())
        {
            sar.push(f64::NAN);
            continue;
        }
...
fn compute_td_ti_028(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_030(series: &LsegSeries<'_>) -> Option<f64> {
    weighted_mean_last(&series.close, 300)
}

fn compute_td_ti_031(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_corr_last(&series.large_order, &series.small_order, 60, 10)
        .ok()
        .flatten()
}

fn compute_td_ti_032(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_rank_last(&range, 60).ok().flatten()
}

fn compute_td_ti_033(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_corr_last(&series.open, &series.volume, 300, 100)
        .ok()
        .flatten()
}

fn compute_td_ti_034(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_035(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_036(series: &LsegSeries<'_>) -> Option<f64> {
    aroon_up_last(&series.high, 14)
}

fn compute_td_ti_037(series: &LsegSeries<'_>) -> Option<f64> {
    aroon_down_last(&series.low, 14)
}

fn compute_td_ti_038(series: &LsegSeries<'_>) -> Option<f64> {
    let up = aroon_up_last(&series.high, 14)?;
    let down = aroon_down_last(&series.low, 14)?;
    finite_opt(Some(up - down))
}

fn compute_td_ti_039(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    ...
    for i in 0..n {
        ...
        let mut valid = max_val.is_finite() && min_val.is_finite();
        for offset in 1..15 {
...
fn compute_td_ti_040(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_min_last(&series.low, 14).ok().flatten()
}

fn compute_td_ti_041(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_max_last(&series.high, 14).ok().flatten()
}

fn compute_td_ti_043(series: &LsegSeries<'_>) -> Option<f64> {
    let curr = current(&series.close)?;
    let mean = rolling_mean_exact(&series.close, 30)?;
    ratio(curr, mean)
}

fn compute_td_ti_044(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_045(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_001(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 10)
}

fn compute_td_mt_002(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.open, 30)
}

fn compute_td_mt_003(series: &LsegSeries<'_>) -> Option<f64> {
    let mom = diff_series(&series.close, 60);
    rolling_mean_opt_exact(&mom, 30)
}

fn compute_td_mt_005(series: &LsegSeries<'_>) -> Option<f64> {
    let fast = rolling_mean_min(&series.close, 12, 1)?;
    let slow = rolling_mean_min(&series.close, 26, 1)?;
    finite_opt(Some(fast - slow))
}

fn compute_td_mt_007(series: &LsegSeries<'_>) -> Option<f64> {
    let close = collect_vec(&series.close);
    if close.len() < 14 {
        return None;
    }
...
fn compute_td_mt_008(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_010(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_011(series: &LsegSeries<'_>) -> Option<f64> {
    let fast = rolling_mean_series(&series.close, 12, 1).ok()?;
    let slow = rolling_mean_series(&series.close, 26, 1).ok()?;
    let macd = binary_opt_vec(&fast, &slow, |a, b| finite_opt(Some(a - b)));
...
fn compute_td_mt_013(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_mt_010(series)
}

fn compute_td_mt_014(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_mt_011(series)
}

fn compute_td_mt_015(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
    ...
    let minus_dm = minus_dm_series(series);
    let tr = tr_series(series);
    let Some(minus) = rolling_mean_exact(&minus_dm, 14) else {
        return Some(f64::NAN);
    };
    ...
    finite_opt(Some(100.0 * minus / tr_mean))
}

fn compute_td_mt_016(series: &LsegSeries<'_>) -> Option<f64> {
    ...
    for i in 1..n {
        if let (Some(curr), Some(prev)) = (tp[i], tp[i - 1]) {
            ...
            if !curr.is_finite() || !prev.is_finite() || !vol.is_finite() {
                pos[i] = Some(f64::NAN);
                neg[i] = Some(f64::NAN);
                continue;
            }
...
fn compute_td_mt_017(series: &LsegSeries<'_>) -> Option<f64> {
    let minus_dm = minus_dm_series(series);
    rolling_mean_exact(&minus_dm, 14)
}

fn compute_td_mt_018(series: &LsegSeries<'_>) -> Option<f64> {
    let tp = zip_map3_opt(&series.high, &series.low, &series.close, |h, l, c| {
        finite_opt(Some((h + l + c) / 3.0))
    });
...
fn compute_td_mt_019(series: &LsegSeries<'_>) -> Option<f64> {
    let curr = current(&series.close)?;
...
fn compute_td_mt_021(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_022(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 1)
}

fn compute_td_mt_025(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    ...
    for value in diff {
        match value {
            Some(v) if !v.is_finite() => {
                gains.push(Some(f64::NAN));
                losses.push(Some(f64::NAN));
            }
            Some(v) if v > 0.0 => {
                gains.push(Some(v));
...
fn compute_td_mt_027(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_028(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_029(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_030(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 1)
}

fn compute_td_mt_032(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_034(series: &LsegSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 30)
}

fn compute_td_mt_035(series: &LsegSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    last_opt(&pct)
}

fn compute_td_mt_036(series: &LsegSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 5)
}

fn compute_td_mt_037(series: &LsegSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 10)
}

fn compute_td_mt_038(series: &LsegSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 14)
}

fn compute_td_mt_039(series: &LsegSeries<'_>) -> Option<f64> {
    let diff = zip_map2(&series.close, &series.low, |c, l| c - l);
    rolling_sum_exact(&diff, 10)
}

fn compute_td_mt_040(series: &LsegSeries<'_>) -> Option<f64> {
    let diff = zip_map2(&series.high, &series.close, |h, c| h - c);
    rolling_sum_exact(&diff, 10)
}

fn compute_td_mt_042(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_tp_vpi_001(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_tp_vpi_002(series: &LsegSeries<'_>) -> Option<f64> {
    let clv = clv_series(series);
    let n = clv.len().min(series.volume.len());
    let mut flow = Vec::with_capacity(n);
    for i in 0..n {
...
fn compute_tp_vpi_003(series: &LsegSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
    ...
    for value in diff {
        match value {
            ...
            Some(_) => {
                up.push(Some(f64::NAN));
                down.push(Some(f64::NAN));
            }
            None => {
                up.push(None);
                down.push(None);
...
fn compute_tp_vpi_004(series: &LsegSeries<'_>) -> Option<f64> {
    let clv = clv_series(series);
    let n = clv.len().min(series.volume.len());
    let mut flow = Vec::with_capacity(n);
    for i in 0..n {
        let volume = series.volume.value_at(i);
        flow.push(match clv[i] {
            Some(value) if value.is_finite() && volume.is_finite() => {
                finite_opt(Some(value * volume))
            }
            Some(_) => Some(f64::NAN),
            None if !volume.is_finite() => Some(f64::NAN),
            None => None,
        });
    }
    let ad = rolling_sum_series_opt(&flow, 360, 360).ok()?;
...
fn compute_tp_vpi_005(series: &LsegSeries<'_>) -> Option<f64> {
    ...
    for i in 0..n {
        let delta = if i == 0 {
            ...
            let volume = series.volume.value_at(i);
            if !curr.is_finite() || !prev.is_finite() || !volume.is_finite() {
                f64::NAN
            } else if curr > prev {
                volume
            } else if curr < prev {
...
fn compute_tp_vpi_006(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    let mut obv = Vec::with_capacity(n);
    for i in 0..n {
        let delta = if i == 0 {
...
fn compute_tp_vpi_008(series: &LsegSeries<'_>) -> Option<f64> {
    let typical = typical_price_series(series);
    let n = typical.len().min(series.volume.len());
    let mut total = Vec::with_capacity(n);
    for i in 0..n {
        let volume = series.volume.value_at(i);
        total.push(match typical[i] {
            Some(value) if value.is_finite() && volume.is_finite() => {
                finite_opt(Some(value * volume)).or(Some(f64::NAN))
            }
            Some(_) | None => Some(f64::NAN),
        });
    }
...
fn compute_tp_vpi_009(series: &LsegSeries<'_>) -> Option<f64> {
    let typical = typical_price_series(series);
...
fn compute_tp_vpi_011(series: &LsegSeries<'_>) -> Option<f64> {
    let short = rolling_mean_exact(&series.volume, 12)?;
    let long = rolling_mean_exact(&series.volume, 26)?;
    finite_opt(Some(short - long))
}

fn compute_tp_vpi_012(series: &LsegSeries<'_>) -> Option<f64> {
    let curr = current(&series.volume)?;
    let mean = rolling_mean_exact(&series.volume, 14)?;
    ratio(curr, mean)
}

fn compute_tp_vpi_013(series: &LsegSeries<'_>) -> Option<f64> {
    let curr = current(&series.volume)?;
    let mean = rolling_mean_exact(&series.volume, 20)?;
    ratio(curr, mean)
}

fn compute_tp_vpi_014(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_tp_vpi_015(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.open.len())
    ...
    finite_opt(Some((close - open) / volume))
}

fn compute_tp_vpi_016(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_tp_vpi_017(series: &LsegSeries<'_>) -> Option<f64> {
    let typical = typical_price_series(series);
    let n = typical
        .len()
        .min(series.close.len())
    ...
    for i in 1..n {
        let current_typical = typical[i];
        let previous_typical = typical[i - 1];
        let current_close = series.close.value_at(i);
        let previous_close = series.close.value_at(i - 1);
        let volume = series.volume.value_at(i);
        if !matches!(current_typical, Some(value) if value.is_finite())
            || !matches!(previous_typical, Some(value) if value.is_finite())
            || !current_close.is_finite()
        ...
        let mf = current_typical.unwrap() * volume;
...
fn compute_tp_vpi_018(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_tp_vpi_019(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_sum_exact(&series.volume, 10)
}

fn compute_td_vi_001(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_vi_002(series: &LsegSeries<'_>) -> Option<f64> {
    sample_std_last(&series.close, 5, 1)
}

fn compute_td_vi_005(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = tr_series(series);
...
fn compute_td_vi_009(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = tr_series(series);
    rolling_std_opt_last(&tr, 14, 1)
}

fn compute_td_vi_010(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_vi_020(series: &LsegSeries<'_>) -> Option<f64> {
    ...
    for i in 0..n {
        ...
        let mut valid =
            min_low.is_finite() && max_high.is_finite() && series.close.value_at(i).is_finite();
        for j in (start + 1)..=i {
            let low = series.low.value_at(j);
            let high = series.high.value_at(j);
            valid &= low.is_finite() && high.is_finite();
            if valid {
        ...
        if !valid {
            fastk_series.push(Some(f64::NAN));
            continue;
        }
...
fn compute_td_vi_021(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_vi_020(series)
}

fn compute_td_vi_022(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_td_vi_023(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_vi_024(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_vi_025(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_vi_026(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
    ...
    for i in 0..n {
        ...
        let low = series.low.value_at(i);
        let close = series.close.value_at(i);
        let open = series.open.value_at(i);
        if !high.is_finite() || !low.is_finite() || !close.is_finite() || !open.is_finite() {
            comp.push(Some(f64::NAN));
            continue;
        }
...
fn compute_td_vi_027(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_max_min(&range, 14, 1)
}

fn compute_td_vi_028(series: &LsegSeries<'_>) -> Option<f64> {
    ...
    for (i, high_part) in upper.iter().enumerate().take(n) {
        ...
        let open = series.open.value_at(i);
        upper_shadow.push(
            if high.is_finite() && close.is_finite() && open.is_finite() {
                finite_opt(Some(high - close.max(open)))
            } else {
                Some(f64::NAN)
            },
...
fn compute_td_vi_029(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .low
        .len()
        .min(series.close.len())
    ...
    for i in 0..n {
        ...
        let open = series.open.value_at(i);
        lower_shadow.push(
            if low.is_finite() && close.is_finite() && open.is_finite() {
                finite_opt(Some(close.min(open) - low))
            } else {
                Some(f64::NAN)
            },
...
fn compute_td_pt_001(series: &LsegSeries<'_>) -> Option<f64> {
    current(&series.close).and_then(|v| finite_opt(Some(v.sin())))
}

fn compute_td_pt_002(series: &LsegSeries<'_>) -> Option<f64> {
    current(&series.close).and_then(|v| finite_opt(Some(v.cos())))
}

fn compute_td_pt_003(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_pt_004(series: &LsegSeries<'_>) -> Option<f64> {
    ...
    for i in 0..n {
        ...
        if !close.is_finite()
            ...
            || !volume.is_finite()
        {
            var1.push(Some(f64::NAN));
            continue;
        }
        let high_low = high - low;
        let vol_mean = vol_ma[i];
        let value = match vol_mean {
            Some(vm) if !vm.is_finite() => Some(f64::NAN),
            Some(vm) if vm.abs() > 1e-12 && high_low.abs() > 1e-12 => {
                finite_opt(Some(((close - open) / high_low) * (volume / vm)))
            }
            Some(_) => Some(0.0),
            None => None,
        };
...
fn compute_td_pt_009(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_pt_010(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_pt_024(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_025(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_026(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
    let open = current(&series.open)?;
...
fn compute_td_pt_027(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_argmin_pos_last(&series.close, 14)
}

fn compute_td_ci_003(series: &LsegSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
...
fn compute_td_ci_007(series: &LsegSeries<'_>) -> Option<f64> {
    let mean = rolling_mean_min(&series.close, 30, 1)?;
    finite_opt(Some(mean.sin()))
}

fn compute_td_ci_008(series: &LsegSeries<'_>) -> Option<f64> {
    let mean = rolling_mean_min(&series.close, 30, 1)?;
    finite_opt(Some(mean.cos()))
}

fn compute_td_ci_010(_series: &LsegSeries<'_>) -> Option<f64> {
    Some(0.0)
}

fn compute_td_pr_001(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_pr_002(series: &LsegSeries<'_>) -> Option<f64> {
    let body = body_last(series)?;
    let upper = upper_shadow_last(series)?;
    let lower = lower_shadow_last(series)?;
...
fn compute_td_pr_005(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.open.len())
    ...
    if [close2, open2, close1, open1, high1, low1, close0, open0]
        .iter()
...
fn compute_td_pr_006(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n < 3 {
        return None;
    }
    if (n - 3..n)
        .any(|i| !series.close.value_at(i).is_finite() || !series.open.value_at(i).is_finite())
    {
        return None;
    }
    Some(
...
fn compute_td_pr_007(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n < 2 {
        return None;
    }
...
fn compute_td_pr_008(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_010(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n < 3 {
        return None;
    }
...
fn compute_td_pr_011(series: &LsegSeries<'_>) -> Option<f64> {
    let body = body_last(series)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_012(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_013(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
...
fn compute_td_pr_014(series: &LsegSeries<'_>) -> Option<f64> {
    let body = body_last(series)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_015(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let low = current(&series.low)?;
    let high = current(&series.high)?;
...
fn compute_td_pr_016(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.open.len())
...
fn compute_td_pr_017(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn td_rsi_last(values: &(impl F64SeriesView + ?Sized), period: usize) -> Option<f64> {
    ...
    let gains: Vec<Option<f64>> = diff
        .iter()
        .map(|value| match value {
            Some(d) if d.is_finite() => Some(if *d > 0.0 { *d } else { 0.0 }),
            Some(_) => Some(f64::NAN),
            None => None,
        })
        .collect();
    let losses: Vec<Option<f64>> = diff
        .iter()
        .map(|value| match value {
            Some(d) if d.is_finite() => Some(if *d < 0.0 { -*d } else { 0.0 }),
            Some(_) => Some(f64::NAN),
...
fn current_tr(series: &LsegSeries<'_>) -> Option<f64> {
    last_opt(&tr_series(series))
}

fn compute_td_ci_001(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    finite_opt(Some(high - low))
}

fn compute_td_ci_002(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    finite_opt(Some(close - open))
}

fn compute_td_ci_004(series: &LsegSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
    let diff_sma = rolling_mean_series_opt(&diff, 30, 1).ok()?;
    let close_sma = rolling_mean_series(&series.close, 30, 1).ok()?;
    let phase = binary_opt_vec(&diff_sma, &close_sma, |d, c| {
...
fn compute_td_ci_005(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_min(&series.close, 30, 1)
}

fn compute_td_ci_006(series: &LsegSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
    let out = rolling_mean_series_opt(&diff, 30, 1).ok()?;
    last_opt(&out)
}

fn compute_td_ci_009(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_min(&series.close, 30, 1)
}

fn compute_td_mt_004(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 3)
}

fn compute_td_mt_006(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_mt_009(series: &LsegSeries<'_>) -> Option<f64> {
    let fast = rolling_mean_min(&series.close, 12, 1)?;
    let slow = rolling_mean_min(&series.close, 26, 1)?;
    finite_opt(Some(fast - slow))
}

fn compute_td_mt_012(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_mt_009(series)
}

fn compute_td_mt_020(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_mt_023(series: &LsegSeries<'_>) -> Option<f64> {
    td_rsi_last(&series.close, 14)
}

fn compute_td_mt_024(series: &LsegSeries<'_>) -> Option<f64> {
    td_rsi_last(&series.volume, 14)
}

fn compute_td_mt_026(series: &LsegSeries<'_>) -> Option<f64> {
    td_rsi_last(&series.close, 14)
}

fn compute_td_mt_031(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_td_mt_033(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 1)
}

fn compute_td_mt_041(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_043(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_exact(&series.volume, 360)
}

fn compute_td_mt_044(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_mean_exact(range.as_slice(), 360)
}

fn compute_td_pr_003(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_004(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_009(series: &LsegSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    Some(if open == close { 1.0 } else { 0.0 })
}

fn compute_td_pt_005(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    finite_opt(Some((high + low) / 2.0))
}

fn compute_td_pt_006(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_007(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_008(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 14)
}

fn compute_td_pt_011(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_pt_012(series: &LsegSeries<'_>) -> Option<f64> {
    let spread = zip_map2(&series.high, &series.low, |h, l| h - l);
    diff_last(spread.as_slice(), 10)
}

fn compute_td_pt_013(series: &LsegSeries<'_>) -> Option<f64> {
    ratio(current(&series.high)?, current(&series.low)?)
}

fn compute_td_pt_014(series: &LsegSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_015(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    ratio(close - open, open)
}

fn compute_td_pt_016(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 14)
}

fn compute_td_pt_017(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 14)
}

fn compute_td_pt_018(series: &LsegSeries<'_>) -> Option<f64> {
    let oc = zip_map2(&series.close, &series.open, |c, o| c - o);
    rolling_mean_exact(oc.as_slice(), 14)
}

fn compute_td_pt_019(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_mean_exact(range.as_slice(), 14)
}

fn compute_td_pt_020(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_max_min(range.as_slice(), 14, 14)
}

fn compute_td_pt_021(series: &LsegSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 14).and_then(|v| finite_opt(Some(v.abs())))
}

fn compute_td_pt_022(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_si_002(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_si_008(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_corr_last(&series.close, &series.volume, 14, 14)
        .ok()
        .flatten()
}

fn compute_td_si_009(series: &LsegSeries<'_>) -> Option<f64> {
    sample_std_last(&series.volume, 14, 14)
}

fn compute_td_si_010(series: &LsegSeries<'_>) -> Option<f64> {
    let tail = collect_vec(&series.close);
    let (slope, intercept) = linear_reg_slope_intercept(&tail[tail.len().saturating_sub(14)..])?;
    finite_opt(Some(slope * 14.0 + intercept))
}

fn compute_td_si_011(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_si_012(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 5, 5)?;
    finite_opt(Some(std * std))
}

fn compute_td_si_013(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_012(series: &LsegSeries<'_>) -> Option<f64> {
    let ema = rolling_mean_series(&series.close, 14, 14).ok()?;
...
fn compute_td_ti_029(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 10 {
        return None;
    }
...
fn compute_td_ti_042(series: &LsegSeries<'_>) -> Option<f64> {
    let high = rolling_max_min(&series.high, 14, 14)?;
    let low = rolling_min_min(&series.low, 14, 14)?;
    finite_opt(Some(high - low))
}

fn compute_td_vi_003(series: &LsegSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_mean_min(range.as_slice(), 14, 1)
}

fn compute_td_vi_004(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = tr_series(series);
    let atr = rolling_mean_series_opt(&tr, 14, 1).ok()?;
    last_opt(&atr)
}

fn compute_td_vi_006(series: &LsegSeries<'_>) -> Option<f64> {
    let atr = compute_td_vi_004(series)?;
    let close = current(&series.close)?;
    ratio(atr * 100.0, close)
}

fn compute_td_vi_007(series: &LsegSeries<'_>) -> Option<f64> {
    current_tr(series)
}

fn compute_td_vi_008(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_vi_004(series)
}

fn compute_td_vi_014(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 10)
}

fn compute_td_vi_015(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let prev = shift_last(&series.close, 10)?;
    ratio(close, prev)
}

fn compute_td_vi_016(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_vi_015(series).and_then(|v| finite_opt(Some(v * 100.0)))
}

fn compute_td_vi_017(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
    ...
    for i in 0..n {
        ...
        let mut valid =
            low_min.is_finite() && high_max.is_finite() && series.close.value_at(i).is_finite();
        for j in start + 1..=i {
            let low = series.low.value_at(j);
            let high = series.high.value_at(j);
            valid &= low.is_finite() && high.is_finite();
            if valid {
                low_min = low_min.min(low);
                high_max = high_max.max(high);
            }
        }
        if !valid {
            slowk.push(Some(f64::NAN));
            continue;
        }
        let close = series.close.value_at(i);
...
fn compute_td_vi_018(series: &LsegSeries<'_>) -> Option<f64> {
    compute_td_vi_017(series)
}

fn compute_tp_vpi_007(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    if n < 30 {
        return None;
    }
...
fn compute_tp_vpi_010(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    if n == 0 {
        return None;
    }
