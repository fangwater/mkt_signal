...
pub fn compute_baseline(name: &str, series: &LsegSeries<'_>) -> Option<f64> {
    match name {
...
fn directional_movement_series(series: &LsegSeries<'_>) -> Option<(Vec<f64>, Vec<f64>)> {
    let n = series.high.len().min(series.low.len());
    if n == 0 {
        return None;
    }
...
fn close_log_return(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 2 {
        return None;
    }
...
fn compute_baseline_023(series: &LsegSeries<'_>) -> Option<f64> {
    close_momentum_oscillator(series, 14, false, false)
}

fn compute_baseline_030(series: &LsegSeries<'_>) -> Option<f64> {
    let short = rolling_mean_last(&series.close, 9).ok().flatten()?;
    let long = rolling_mean_last(&series.close, 150).ok().flatten()?;
    finite_opt(Some(short + long))
}

fn compute_baseline_033(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_difference(&series.close, 12, 26)
}

fn compute_baseline_037(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_high_low_range(series, 14)
}

fn compute_baseline_041(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_difference(&series.volume, 12, 26)
}

fn compute_baseline_044(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_048(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.active_buy_ratio_5m, 30)
        .ok()
        .flatten()
}

fn compute_baseline_050(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_range_mean(series, 10, 10)
}

fn compute_baseline_056(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_difference(&series.volume, 12, 26)
}

fn compute_baseline_064(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_difference(&series.close, 12, 26)
}

fn compute_baseline_075(series: &LsegSeries<'_>) -> Option<f64> {
    activity_ratio(&series.large_pct_30m, 300)
}

fn compute_baseline_078(series: &LsegSeries<'_>) -> Option<f64> {
    activity_ratio(&series.large_pct_120m, 300)
}

fn compute_baseline_084(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_089(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_difference(&series.close, 12, 26)
}

fn compute_baseline_094(series: &LsegSeries<'_>) -> Option<f64> {
    activity_ratio(&series.small_pct_30m, 300)
}

fn compute_baseline_095(series: &LsegSeries<'_>) -> Option<f64> {
    activity_ratio(&series.small_pct_120m, 300)
}

fn compute_baseline_097(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.close, 45).ok().flatten()
}

fn compute_baseline_102(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.net_buy_small_pct_15m, 120)
        .ok()
        .flatten()
}

fn compute_baseline_106(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_108(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 1)
}

fn compute_baseline_130(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_range_mean(series, 14, 1)
}

fn compute_baseline_142(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_high_low_range(series, 14)
}

fn compute_baseline_144(series: &LsegSeries<'_>) -> Option<f64> {
    finite_opt(Some(
        current_value(&series.close)? - current_value(&series.open)?,
    ))
}

fn compute_baseline_147(series: &LsegSeries<'_>) -> Option<f64> {
    close_momentum_oscillator(series, 14, true, false)
}

fn compute_baseline_150(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_155(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.active_buy_ratio_240m, 150)
        .ok()
        .flatten()
}

fn compute_baseline_165(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 1)
}

fn compute_baseline_176(series: &LsegSeries<'_>) -> Option<f64> {
    let close = current_value(&series.close)?;
    let open = current_value(&series.open)?;
    safe_ratio(close - open, open)
}

fn compute_baseline_183(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n == 0 {
        return None;
    }
...
fn compute_baseline_186(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_range_mean(series, 14, 14)
}

fn compute_baseline_197(series: &LsegSeries<'_>) -> Option<f64> {
    close_momentum_oscillator(series, 14, true, true)
}

fn compute_baseline_001(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.close, 30).ok().flatten()
}

fn compute_baseline_002(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.close, 30).ok().flatten()
}

fn compute_baseline_003(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_weighted_mean_last(&series.close, 30)
}

fn compute_baseline_004(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_005(series: &LsegSeries<'_>) -> Option<f64> {
    rsi_last_from_series(&series.close, 60)
}

fn compute_baseline_006(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_007(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_008(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_013(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_014(series: &LsegSeries<'_>) -> Option<f64> {
    cmo_last_from_series(&series.close, 14)
}

fn compute_baseline_015(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.large_order, 10)
}

fn compute_baseline_016(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_017(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = build_true_range_series(series)?;
    rolling_mean_last_opt_from_series(&tr, 14, 14)
        .ok()
        .flatten()
...
fn compute_baseline_018(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_019(series: &LsegSeries<'_>) -> Option<f64> {
    if series.close.len() <= 3 {
        return None;
    }
...
fn compute_baseline_020(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_021(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_022(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_024(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = build_true_range_series(series)?;
    last_opt(&tr)
}

fn compute_baseline_025(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
...
fn compute_baseline_026(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_027(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 4 {
        return None;
    }
...
fn compute_baseline_028(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n <= 30 {
        return None;
    }
...
fn compute_baseline_029(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_031(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_032(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_034(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n == 0 {
        return None;
    }
...
fn compute_baseline_035(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_036(series: &LsegSeries<'_>) -> Option<f64> {
    let fast = rolling_weighted_mean_last(&series.close, 12)?;
    let slow = rolling_weighted_mean_last(&series.close, 26)?;
    finite_opt(Some(fast - slow))
}

fn compute_baseline_038(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 15 {
        return None;
    }
...
fn compute_baseline_039(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 15 {
        return None;
    }
...
fn compute_baseline_040(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_042(series: &LsegSeries<'_>) -> Option<f64> {
    rsi_last_from_series(&series.volume, 14)
}

fn compute_baseline_043(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = build_true_range_series(series)?;
...
fn compute_baseline_045(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_046(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_047(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_049(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.small_buy.len().min(series.small_sell.len());
    if n < 150 {
        return None;
    }
...
fn compute_baseline_051(series: &LsegSeries<'_>) -> Option<f64> {
    let mean = rolling_mean_last(&series.close, 10).ok().flatten()?;
    finite_opt(Some(series.close[series.close.len() - 1] - mean))
}

fn compute_baseline_052(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_053(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_054(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_055(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_057(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_058(series: &LsegSeries<'_>) -> Option<f64> {
    sample_std_last(&series.volume, 14, 14)
}

fn compute_baseline_059(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 20, 20)?;
    finite_opt(Some(4.0 * std))
}

fn compute_baseline_060(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n <= 10 {
        return None;
    }
...
fn compute_baseline_061(series: &LsegSeries<'_>) -> Option<f64> {
    let short = rolling_mean_last(&series.close, 12).ok().flatten()?;
    let long = rolling_mean_last(&series.close, 26).ok().flatten()?;
    finite_opt(Some(short - long))
}

fn compute_baseline_062(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n == 0 {
        return None;
    }
...
fn compute_baseline_063(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_065(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .open
        .len()
        .min(series.high.len())
...
fn compute_baseline_066(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_067(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_068(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_069(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_070(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_071(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_072(series: &LsegSeries<'_>) -> Option<f64> {
    sample_std_last(&series.close, 5, 5)
}

fn compute_baseline_073(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 5, 5)?;
    finite_opt(Some(std * std))
}

fn compute_baseline_074(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 10 {
        return None;
    }
...
fn compute_baseline_076(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = build_true_range_series(series)?;
...
fn compute_baseline_077(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_079(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_080(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    let window = 10usize;
    if n < window {
        return None;
...
fn compute_baseline_081(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.close.len())
...
fn compute_baseline_082(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.low.len())
...
fn compute_baseline_083(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_position_last(&series.close, 14)
}

fn compute_baseline_085(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_086(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_087(series: &LsegSeries<'_>) -> Option<f64> {
    let ad = build_ad_line_rolling360(series)?;
    last_opt(&ad)
}

fn compute_baseline_088(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_090(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_091(series: &LsegSeries<'_>) -> Option<f64> {
    let obv = build_obv_cumulative(series)?;
    finite_opt(Some(*obv.last()?))
}

fn compute_baseline_092(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_position_last(&series.close, 10)
}

fn compute_baseline_093(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_096(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.close, 5).ok().flatten()
}

fn compute_baseline_098(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_099(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_position_last(&series.close, 14)
}

fn compute_baseline_100(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_mean_last(&series.close, 180).ok().flatten()
}

fn compute_baseline_101(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_103(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_104(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 60 {
        return None;
    }
...
fn compute_baseline_105(series: &LsegSeries<'_>) -> Option<f64> {
    if series.close.len() <= 14 {
        return None;
    }
    finite_opt(Some(
...
fn compute_baseline_107(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_109(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.open.len())
...
fn compute_baseline_110(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .low
        .len()
        .min(series.open.len())
...
fn compute_baseline_111(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_sum_last_with_min_periods(&series.volume, 10, 10)
        .ok()
        .flatten()
}

fn compute_baseline_112(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 14)
}

fn compute_baseline_113(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 90 {
        return None;
    }
...
fn compute_baseline_114(series: &LsegSeries<'_>) -> Option<f64> {
    if series.close.len() <= 10 {
        return None;
    }
...
fn compute_baseline_115(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.open.len())
...
fn compute_baseline_116(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_117(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 26 {
        return None;
    }
...
fn compute_baseline_118(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 143 {
        return None;
    }
...
fn compute_baseline_119(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_120(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_121(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_122(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_123(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_124(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_125(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 20 {
        return None;
    }
...
fn compute_baseline_126(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
...
fn compute_baseline_127(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 14, 14)?;
    finite_opt(Some(-2.0 * std))
}

fn compute_baseline_128(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_129(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 28 {
        return None;
    }
...
fn compute_baseline_131(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_132(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_133(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_134(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_135(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_136(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_137(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.open.len())
...
fn compute_baseline_138(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_139(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_140(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_141(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.close.len());
    if n < 60 {
        return None;
    }
...
fn compute_baseline_143(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n == 0 {
        return None;
    }
...
fn compute_baseline_145(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_146(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n < 19 {
        return None;
    }
...
fn compute_baseline_148(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n == 0 {
        return None;
    }
...
fn compute_baseline_149(series: &LsegSeries<'_>) -> Option<f64> {
    let ad = build_ad_line_rolling360(series)?;
...
fn compute_baseline_151(series: &LsegSeries<'_>) -> Option<f64> {
    compute_tp_vpi_006(series)
}

fn compute_baseline_152(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 34 {
        return None;
    }
...
fn compute_baseline_153(series: &LsegSeries<'_>) -> Option<f64> {
    let tr = build_true_range_series(series)?;
    let n = tr.len();
    if n < 15 {
        return None;
...
fn compute_baseline_154(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_156(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n == 0 {
        return None;
    }
...
fn compute_baseline_157(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n <= 30 {
        return None;
    }
...
fn compute_baseline_158(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_159(series: &LsegSeries<'_>) -> Option<f64> {
    close_log_return(series)
        .map(f64::sin)
        .and_then(|v| finite_opt(Some(v)))
}

fn compute_baseline_160(series: &LsegSeries<'_>) -> Option<f64> {
    close_log_return(series)
        .map(f64::cos)
        .and_then(|v| finite_opt(Some(v)))
}

fn compute_baseline_161(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_162(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_163(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 30 {
        return None;
    }
...
fn compute_baseline_164(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 30 {
        return None;
    }
...
fn compute_baseline_166(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 5)
}

fn compute_baseline_167(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 10)
}

fn compute_baseline_168(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len();
    if n < 35 {
        return None;
    }
...
fn compute_baseline_169(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_170(series: &LsegSeries<'_>) -> Option<f64> {
    let ad = build_ad_line_rolling360(series)?;
    last_opt(&ad)
}

fn compute_baseline_171(series: &LsegSeries<'_>) -> Option<f64> {
    let obv = build_obv_cumulative(series)?;
    finite_opt(Some(*obv.last()?))
}

fn compute_baseline_172(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_173(series: &LsegSeries<'_>) -> Option<f64> {
    if series.close.len() <= 14 {
        return None;
    }
    finite_opt(Some(
...
fn compute_baseline_174(series: &LsegSeries<'_>) -> Option<f64> {
    let high = rolling_max_last(&series.close, 14)?;
    let low = rolling_min_last(&series.close, 14)?;
    finite_opt(Some(high - low))
}

fn compute_baseline_175(series: &LsegSeries<'_>) -> Option<f64> {
    let mean = rolling_mean_last(&series.close, 14).ok().flatten()?;
    finite_opt(Some((series.close[series.close.len() - 1] - mean).abs()))
}

fn compute_baseline_177(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_178(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_179(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_180(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    if n < 30 {
        return None;
    }
...
fn compute_baseline_181(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_182(series: &LsegSeries<'_>) -> Option<f64> {
    let mean = rolling_mean_last(&series.volume, 14).ok().flatten()?;
    if mean.abs() <= 1e-12 {
        return Some(0.0);
    }
...
fn compute_baseline_184(series: &LsegSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 14)
}

fn compute_baseline_185(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_187(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.open.len())
...
fn compute_baseline_188(series: &LsegSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 20, 20)?;
    finite_opt(Some(4.0 * std))
}

fn compute_baseline_189(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_190(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_191(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_baseline_192(series: &LsegSeries<'_>) -> Option<f64> {
    rolling_position_last(&series.close, 14)
}

fn compute_baseline_193(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.buy_amount.len().min(series.sell_amount.len());
    if n == 0 {
        return None;
    }
...
fn compute_baseline_194(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.buy_amount.len().min(series.sell_amount.len());
    if n == 0 {
        return None;
    }
...
fn compute_baseline_195(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_196(series: &LsegSeries<'_>) -> Option<f64> {
    let periods = 10usize;
...
fn compute_baseline_198(series: &LsegSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 14 {
        return None;
    }
...
fn compute_baseline_199(series: &LsegSeries<'_>) -> Option<f64> {
...
fn compute_baseline_200(series: &LsegSeries<'_>) -> Option<f64> {
