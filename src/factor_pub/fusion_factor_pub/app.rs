...
use super::cfg::{FusionFactorPubConfig, FusionFactorVersion, TlenServerConfig};
...
impl FusionFactorPubApp {
    pub async fn new(config_path: &str, venue: TradingVenue) -> Result<Self> {
        Self::new_for_version(config_path, venue, FusionFactorVersion::Default)
            .await?
    ...
    pub async fn try_new_one_minute(
        ...
        Self::new_for_version(config_path, venue, FusionFactorVersion::OneMinute).await
    }
    ...
    async fn new_for_version(
        ...
        if allowed_symbols.is_empty() {
            if version == FusionFactorVersion::OneMinute {
                info!(
                    "FusionFactorPubApp[{}:1m] disabled: config_type={} has no online symbols",
                    venue_slug,
                    version.amount_threshold_config_type()
                );
                return Ok(None);
        ...
        if version == FusionFactorVersion::OneMinute && symbol_factor_plans.is_empty() {
            info!(
                "FusionFactorPubApp[{}:1m] disabled: config_type={} is empty",
                venue_slug,
                version.factor_plan_config_type()
            );
            return Ok(None);
        ...
        let zscore_config = if version == FusionFactorVersion::OneMinute {
        ...
        info!(
            "FusionFactorPubApp created: venue={} version={} symbols={} sample={} factor_plan_config={} zscore_config={} symbol_config={} trade_flow_channel=factor_pub/{}/{} output_service={}",
            venue_slug,
            version.label(),
            allowed_symbols.len(),
        ...
        info!(
            "FusionFactorPubApp[{}:{}] zscore config: window_size={} min_samples={} zscore_cap={}",
            venue_slug,
            version.label(),
    ...
    fn prepare_run(&mut self) -> Result<()> {
        ...
        info!(
            "FusionFactorPubApp[{}:{}] started: symbols={} sample={} drained_stale={}",
            self.venue_slug,
            self.version.label(),
            self.allowed_symbols.len(),
            format_symbol_sample_set(&self.allowed_symbols),
    ...
    fn compute_extra_factor(
        ...
        match extra_factor_id {
            ...
            ExtraFactorId::Open => finite_opt(series.open.last().copied()),
            ExtraFactorId::High => finite_opt(series.high.last().copied()),
            ExtraFactorId::Low => finite_opt(series.low.last().copied()),
            ExtraFactorId::Close => finite_opt(series.close.last().copied()),
            ExtraFactorId::Volume => finite_opt(series.volume.last().copied()),
            ExtraFactorId::Amount => finite_opt(series.amount.last().copied()),
            ExtraFactorId::AvgAmount => finite_opt(series.avg_amount.last().copied()),
            ExtraFactorId::Count => finite_opt(series.count.last().copied()),
            ExtraFactorId::BuyCount => finite_opt(series.buy_count.last().copied()),
            ExtraFactorId::SellCount => finite_opt(series.sell_count.last().copied()),
            ExtraFactorId::BuyAmount => finite_opt(series.buy_amount.last().copied()),
            ExtraFactorId::SellAmount => finite_opt(series.sell_amount.last().copied()),
            ExtraFactorId::BuyVolume => finite_opt(series.buy_volume.last().copied()),
            ExtraFactorId::SellVolume => finite_opt(series.sell_volume.last().copied()),
            ExtraFactorId::LargeOrder => finite_opt(series.large_order.last().copied()),
            ExtraFactorId::MediumOrder => finite_opt(series.medium_order.last().copied()),
            ExtraFactorId::SmallOrder => finite_opt(series.small_order.last().copied()),
            ExtraFactorId::LargeBuy => finite_opt(series.large_buy.last().copied()),
            ExtraFactorId::LargeSell => finite_opt(series.large_sell.last().copied()),
            ExtraFactorId::MediumBuy => finite_opt(series.medium_buy.last().copied()),
            ExtraFactorId::MediumSell => finite_opt(series.medium_sell.last().copied()),
            ExtraFactorId::Vwap => finite_opt(series.vwap.last().copied()),
            ExtraFactorId::BuyVwap => finite_opt(series.buy_vwap.last().copied()),
            ExtraFactorId::SellVwap => finite_opt(series.sell_vwap.last().copied()),
            ExtraFactorId::NetBuyAmount => finite_opt(series.net_buy_amount.last().copied()),
            ExtraFactorId::NetBuyVolume => finite_opt(series.net_buy_volume.last().copied()),
