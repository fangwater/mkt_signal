...
pub enum CnFactorVersion {
    Default,
    OneMinute,
}

impl CnFactorVersion {
...
pub struct CnFactorEngineConfig {
...
impl CnFactorEngineConfig {
    pub fn load(path: &str) -> Result<Self> {
        let content = fs::read_to_string(path)?;
        let cfg: CnFactorEngineConfig = toml::from_str(&content)?;
        cfg.validate()?;
        Ok(cfg)
    }
...
#[cfg(test)]
mod tests {
    use super::CnFactorVersion;

    #[test]
    fn one_minute_version_uses_isolated_resources() {
        let version = CnFactorVersion::OneMinute;
