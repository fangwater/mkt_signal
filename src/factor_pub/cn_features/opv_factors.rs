    ...
    ($binding:expr, $series:expr, { $($variant:ident => $func:ident,)* }) => {
        match $binding.factor_id {
            $(Some(CnFactorId::$variant) => {
                Some(CnFactorEngine::wrap_factor_value($series.and_then($func)))
            })*
            _ => None,
        }
    };
...
fn clv_series(series: &CnSeries<'_>) -> Vec<Option<f64>> {
    zip_map3_opt(
        &series.close,
        &series.low,
        &series.high,
...
fn typical_price_series(series: &CnSeries<'_>) -> Vec<Option<f64>> {
    zip_map3_opt(
        &series.high,
        &series.low,
        &series.close,
...
fn tr_series(series: &CnSeries<'_>) -> Vec<Option<f64>> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn bp_series(series: &CnSeries<'_>) -> Vec<Option<f64>> {
...
fn plus_dm_series(series: &CnSeries<'_>) -> Vec<f64> {
...
fn minus_dm_series(series: &CnSeries<'_>) -> Vec<f64> {
    let n = series.high.len().min(series.low.len());
    let mut out = Vec::with_capacity(n);
    out.push(0.0);
    for i in 1..n {
...
fn body_last(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    finite_opt(Some((close - open).abs()))
}

fn upper_shadow_last(series: &CnSeries<'_>) -> Option<f64> {
...
fn lower_shadow_last(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_011(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_013(series: &CnSeries<'_>) -> Option<f64> {
    let pv = zip_map2(&series.close, &series.volume, |c, v| c * v);
    rolling_mean_exact(&pv, 30)
}

fn compute_td_ti_014(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_016(series: &CnSeries<'_>) -> Option<f64> {
    let middle = rolling_mean_exact(&series.close, 20)?;
    let std = sample_std_last(&series.close, 20, 20)?;
    finite_opt(Some(middle - 2.0 * std))
}

fn compute_td_ti_017(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_018(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_024(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_025(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_027(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_ti_028(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_030(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_032(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_035(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_038(series: &CnSeries<'_>) -> Option<f64> {
    let up = aroon_up_last(&series.high, 14)?;
    let down = aroon_down_last(&series.low, 14)?;
    finite_opt(Some(up - down))
}

fn compute_td_ti_039(series: &CnSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
...
fn compute_td_ti_040(series: &CnSeries<'_>) -> Option<f64> {
    rolling_min_last(&series.low, 14).ok().flatten()
}

fn compute_td_ti_041(series: &CnSeries<'_>) -> Option<f64> {
    rolling_max_last(&series.high, 14).ok().flatten()
}

fn compute_td_ti_043(series: &CnSeries<'_>) -> Option<f64> {
    let curr = current(&series.close)?;
    let mean = rolling_mean_exact(&series.close, 30)?;
    ratio(curr, mean)
}

fn compute_td_ti_044(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_045(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_001(series: &CnSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 10)
}

fn compute_td_mt_002(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_010(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_011(series: &CnSeries<'_>) -> Option<f64> {
    let fast = rolling_mean_series(&series.close, 12, 1).ok()?;
    let slow = rolling_mean_series(&series.close, 26, 1).ok()?;
    let macd = binary_opt_vec(&fast, &slow, |a, b| finite_opt(Some(a - b)));
...
fn compute_td_mt_013(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_016(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_017(series: &CnSeries<'_>) -> Option<f64> {
    let minus_dm = minus_dm_series(series);
    rolling_mean_exact(&minus_dm, 14)
}

fn compute_td_mt_018(series: &CnSeries<'_>) -> Option<f64> {
    let tp = zip_map3_opt(&series.high, &series.low, &series.close, |h, l, c| {
        finite_opt(Some((h + l + c) / 3.0))
    });
...
fn compute_td_mt_019(series: &CnSeries<'_>) -> Option<f64> {
    let curr = current(&series.close)?;
...
fn compute_td_mt_021(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_022(series: &CnSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 1)
}

fn compute_td_mt_025(series: &CnSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
...
fn compute_td_mt_027(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_028(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_029(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_030(series: &CnSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 1)
}

fn compute_td_mt_032(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_034(series: &CnSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 30)
}

fn compute_td_mt_035(series: &CnSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    last_opt(&pct)
}

fn compute_td_mt_036(series: &CnSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 5)
}

fn compute_td_mt_037(series: &CnSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
    rolling_mean_opt_exact(&pct, 10)
}

fn compute_td_mt_038(series: &CnSeries<'_>) -> Option<f64> {
    let pct = pct_change_series(&series.close, 5);
...
fn compute_td_mt_040(series: &CnSeries<'_>) -> Option<f64> {
    let diff = zip_map2(&series.high, &series.close, |h, c| h - c);
    rolling_sum_exact(&diff, 10)
}

fn compute_td_mt_042(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_tp_vpi_003(series: &CnSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
...
fn compute_tp_vpi_008(series: &CnSeries<'_>) -> Option<f64> {
    let typical = typical_price_series(series);
...
fn compute_tp_vpi_009(series: &CnSeries<'_>) -> Option<f64> {
    let typical = typical_price_series(series);
...
fn compute_tp_vpi_016(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_tp_vpi_018(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .high
        .len()
        .min(series.low.len())
...
fn compute_tp_vpi_019(series: &CnSeries<'_>) -> Option<f64> {
    rolling_sum_exact(&series.volume, 10)
}

fn compute_td_vi_001(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_vi_002(series: &CnSeries<'_>) -> Option<f64> {
    sample_std_last(&series.close, 5, 1)
}

fn compute_td_vi_005(series: &CnSeries<'_>) -> Option<f64> {
    let tr = tr_series(series);
...
fn compute_td_vi_009(series: &CnSeries<'_>) -> Option<f64> {
    let tr = tr_series(series);
...
fn compute_td_vi_021(series: &CnSeries<'_>) -> Option<f64> {
    compute_td_vi_020(series)
}

fn compute_td_vi_022(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_td_vi_023(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_vi_024(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_vi_027(series: &CnSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
...
fn compute_td_vi_029(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .low
        .len()
        .min(series.close.len())
...
fn compute_td_pt_009(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_pt_024(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_025(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_026(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
    let open = current(&series.open)?;
...
fn compute_td_ci_010(_series: &CnSeries<'_>) -> Option<f64> {
    Some(0.0)
}
...
fn compute_td_pr_008(series: &CnSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_010(series: &CnSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
    if n < 3 {
        return None;
    }
...
fn compute_td_pr_011(series: &CnSeries<'_>) -> Option<f64> {
    let body = body_last(series)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_012(series: &CnSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_013(series: &CnSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.open.len());
...
fn compute_td_pr_014(series: &CnSeries<'_>) -> Option<f64> {
    let body = body_last(series)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn current_tr(series: &CnSeries<'_>) -> Option<f64> {
    last_opt(&tr_series(series))
}

fn compute_td_ci_001(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    finite_opt(Some(high - low))
}

fn compute_td_ci_002(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    finite_opt(Some(close - open))
}

fn compute_td_ci_004(series: &CnSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
    let diff_sma = rolling_mean_series_opt(&diff, 30, 1).ok()?;
    let close_sma = rolling_mean_series(&series.close, 30, 1).ok()?;
    let phase = binary_opt_vec(&diff_sma, &close_sma, |d, c| {
...
fn compute_td_ci_005(series: &CnSeries<'_>) -> Option<f64> {
    rolling_mean_min(&series.close, 30, 1)
}

fn compute_td_ci_006(series: &CnSeries<'_>) -> Option<f64> {
    let diff = diff_series(&series.close, 1);
    let out = rolling_mean_series_opt(&diff, 30, 1).ok()?;
    last_opt(&out)
}

fn compute_td_ci_009(series: &CnSeries<'_>) -> Option<f64> {
    rolling_mean_min(&series.close, 30, 1)
}

fn compute_td_mt_004(series: &CnSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 3)
}

fn compute_td_mt_006(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_mt_009(series: &CnSeries<'_>) -> Option<f64> {
    let fast = rolling_mean_min(&series.close, 12, 1)?;
    let slow = rolling_mean_min(&series.close, 26, 1)?;
    finite_opt(Some(fast - slow))
}

fn compute_td_mt_012(series: &CnSeries<'_>) -> Option<f64> {
    compute_td_mt_009(series)
}

fn compute_td_mt_020(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_mt_023(series: &CnSeries<'_>) -> Option<f64> {
    td_rsi_last(&series.close, 14)
}

fn compute_td_mt_024(series: &CnSeries<'_>) -> Option<f64> {
    td_rsi_last(&series.volume, 14)
}

fn compute_td_mt_026(series: &CnSeries<'_>) -> Option<f64> {
    td_rsi_last(&series.close, 14)
}

fn compute_td_mt_031(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
...
fn compute_td_mt_033(series: &CnSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 1)
}

fn compute_td_mt_041(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_mt_043(series: &CnSeries<'_>) -> Option<f64> {
    rolling_mean_exact(&series.volume, 360)
}

fn compute_td_mt_044(series: &CnSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_mean_exact(range.as_slice(), 360)
}

fn compute_td_pr_003(series: &CnSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_004(series: &CnSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    let high = current(&series.high)?;
    let low = current(&series.low)?;
...
fn compute_td_pr_009(series: &CnSeries<'_>) -> Option<f64> {
    let open = current(&series.open)?;
    let close = current(&series.close)?;
    Some(if open == close { 1.0 } else { 0.0 })
}

fn compute_td_pt_005(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    finite_opt(Some((high + low) / 2.0))
}

fn compute_td_pt_006(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_007(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_008(series: &CnSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 14)
}

fn compute_td_pt_011(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_pt_012(series: &CnSeries<'_>) -> Option<f64> {
    let spread = zip_map2(&series.high, &series.low, |h, l| h - l);
    diff_last(spread.as_slice(), 10)
}

fn compute_td_pt_013(series: &CnSeries<'_>) -> Option<f64> {
    ratio(current(&series.high)?, current(&series.low)?)
}

fn compute_td_pt_014(series: &CnSeries<'_>) -> Option<f64> {
    let high = current(&series.high)?;
    let low = current(&series.low)?;
    let close = current(&series.close)?;
...
fn compute_td_pt_015(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let open = current(&series.open)?;
    ratio(close - open, open)
}

fn compute_td_pt_016(series: &CnSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 14)
}

fn compute_td_pt_017(series: &CnSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 14)
}

fn compute_td_pt_018(series: &CnSeries<'_>) -> Option<f64> {
    let oc = zip_map2(&series.close, &series.open, |c, o| c - o);
    rolling_mean_exact(oc.as_slice(), 14)
}

fn compute_td_pt_019(series: &CnSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_mean_exact(range.as_slice(), 14)
}

fn compute_td_pt_020(series: &CnSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_max_min(range.as_slice(), 14, 14)
}

fn compute_td_pt_021(series: &CnSeries<'_>) -> Option<f64> {
    diff_last(&series.close, 14).and_then(|v| finite_opt(Some(v.abs())))
}

fn compute_td_pt_022(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_si_002(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_si_008(series: &CnSeries<'_>) -> Option<f64> {
    rolling_corr_last(&series.close, &series.volume, 14, 14)
        .ok()
        .flatten()
}

fn compute_td_si_009(series: &CnSeries<'_>) -> Option<f64> {
    sample_std_last(&series.volume, 14, 14)
}

fn compute_td_si_010(series: &CnSeries<'_>) -> Option<f64> {
    let tail = collect_vec(&series.close);
    let (slope, intercept) = linear_reg_slope_intercept(&tail[tail.len().saturating_sub(14)..])?;
    finite_opt(Some(slope * 14.0 + intercept))
}

fn compute_td_si_011(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_si_012(series: &CnSeries<'_>) -> Option<f64> {
    let std = sample_std_last(&series.close, 5, 5)?;
    finite_opt(Some(std * std))
}

fn compute_td_si_013(series: &CnSeries<'_>) -> Option<f64> {
...
fn compute_td_ti_012(series: &CnSeries<'_>) -> Option<f64> {
    let ema = rolling_mean_series(&series.close, 14, 14).ok()?;
...
fn compute_td_ti_029(series: &CnSeries<'_>) -> Option<f64> {
    let n = series.high.len().min(series.low.len());
    if n < 10 {
        return None;
    }
...
fn compute_td_ti_042(series: &CnSeries<'_>) -> Option<f64> {
    let high = rolling_max_min(&series.high, 14, 14)?;
    let low = rolling_min_min(&series.low, 14, 14)?;
    finite_opt(Some(high - low))
}

fn compute_td_vi_003(series: &CnSeries<'_>) -> Option<f64> {
    let range = zip_map2(&series.high, &series.low, |h, l| h - l);
    rolling_mean_min(range.as_slice(), 14, 1)
}

fn compute_td_vi_004(series: &CnSeries<'_>) -> Option<f64> {
    let tr = tr_series(series);
    let atr = rolling_mean_series_opt(&tr, 14, 1).ok()?;
    last_opt(&atr)
}

fn compute_td_vi_006(series: &CnSeries<'_>) -> Option<f64> {
    let atr = compute_td_vi_004(series)?;
    let close = current(&series.close)?;
    ratio(atr * 100.0, close)
}

fn compute_td_vi_007(series: &CnSeries<'_>) -> Option<f64> {
    current_tr(series)
}

fn compute_td_vi_008(series: &CnSeries<'_>) -> Option<f64> {
    compute_td_vi_004(series)
}

fn compute_td_vi_014(series: &CnSeries<'_>) -> Option<f64> {
    pct_change_last(&series.close, 10)
}

fn compute_td_vi_015(series: &CnSeries<'_>) -> Option<f64> {
    let close = current(&series.close)?;
    let prev = shift_last(&series.close, 10)?;
    ratio(close, prev)
}

fn compute_td_vi_016(series: &CnSeries<'_>) -> Option<f64> {
    compute_td_vi_015(series).and_then(|v| finite_opt(Some(v * 100.0)))
}

fn compute_td_vi_017(series: &CnSeries<'_>) -> Option<f64> {
    let n = series
        .close
        .len()
        .min(series.high.len())
...
fn compute_td_vi_018(series: &CnSeries<'_>) -> Option<f64> {
    compute_td_vi_017(series)
}

fn compute_tp_vpi_007(series: &CnSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    if n < 30 {
        return None;
    }
...
fn compute_tp_vpi_010(series: &CnSeries<'_>) -> Option<f64> {
    let n = series.close.len().min(series.volume.len());
    if n == 0 {
        return None;
    }
