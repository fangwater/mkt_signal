...
pub struct CnFormulaPlan {
    pub(crate) ordered_factors: Vec<CnFactorBinding>,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct CnFactorBinding {
    pub(crate) name: String,
    pub(crate) factor_id: Option<CnFactorId>,
    pub(crate) extra_factor_id: Option<CnExtraFactorId>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CnExtraFactorId {
    AvgPrice,
    BuyAvgPrice,
    SellAvgPrice,
    SmallBuy,
...
fn build_factor_plan(scope: &str, factors: Vec<String>) -> Result<CnFormulaPlan> {
    let mut ordered_factors = Vec::with_capacity(factors.len());
    for name in factors {
        let factor_id = CnFactorId::from_name(&name);
        let extra_factor_id = CnExtraFactorId::from_name(&name);
        if factor_id.is_none() && extra_factor_id.is_none() {
            bail!("factor-plan unmapped: {} {}", scope, name);
        }
        ordered_factors.push(CnFactorBinding {
            name,
            factor_id,
            extra_factor_id,
        });
    ...
    Ok(CnFormulaPlan { ordered_factors })
}

fn decode_symbol_factor_plans(payload: FactorPlanResp) -> Result<HashMap<String, CnFormulaPlan>> {
...
pub(crate) async fn load_symbol_factor_plans_from_tlen_server(
    ...
    venue_slug: &str,
    config_type: &str,
) -> Result<HashMap<String, CnFormulaPlan>> {
...
mod tests {
    ...
    fn public_factor_plan_constructor_validates_names_and_preserves_order() {
        ...
        assert!(
            CnFormulaPlan::from_factor_names("BTCUSDT", vec!["not_a_factor".to_string()]).is_err()
        );
    }
    ...
    fn all_trade_flow_feature_fields_have_extra_factor_ids() {
        for name in TRADE_FLOW_FEATURE_FIELD_NAMES {
            assert!(
                CnExtraFactorId::from_name(name).is_some(),
                "missing CnExtraFactorId mapping for {}",
                name
            );
        }
    }
    ...
    fn level_one_depth_fields_have_stable_extra_factor_ids() {
        let expected = [
            ("bid0p_1", CnExtraFactorId::Bid0Price1),
            ("bid0v_1", CnExtraFactorId::Bid0Volume1),
            ("ask0p_1", CnExtraFactorId::Ask0Price1),
            ("ask0v_1", CnExtraFactorId::Ask0Volume1),
        ];

        for (name, factor_id) in expected {
            assert_eq!(CnExtraFactorId::from_name(name), Some(factor_id));
            assert_eq!(
                CnExtraFactorId::index_to_name(factor_id.as_index()),
                Some(name)
            );
        }
    }
