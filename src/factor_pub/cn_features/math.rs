...
use super::view::{F64SeriesView, OptF64SeriesView};
...
pub fn rolling_mean_last(
    ...
    Ok(finite_or_none(Some(mean(values, start, end))))
}
...
pub fn rolling_std_last(
    ...
    let mean = mean(values, start, end);
    let mut squared_deviation = 0.0;
    for index in start..end {
        let deviation = values.value_at(index) - mean;
        squared_deviation += deviation * deviation;
    }
    Ok(finite_or_none(Some(
...
pub fn rolling_mean_at_opt_from_series(
    ...
    match values.value_at(end - 1) {
        Some(value) if !value.is_finite() => return Ok(Some(f64::NAN)),
        Some(_) => {}
        None => return Ok(None),
    }
    let start = end.saturating_sub(window);
...
pub fn rolling_mean_series_opt(
    ...
    for i in 0..n {
        ...
        match values.value_at(i) {
            Some(value) if !value.is_finite() => out[i] = Some(f64::NAN),
            Some(_) if valid >= min_periods => {
...
pub fn rolling_skew_last(
    ...
    Ok(skew_from_range(values, start, end, bias))
}
...
pub fn rolling_kurt_last(
    ...
    Ok(kurtosis_from_range(values, start, end, fisher, bias))
}
...
pub fn tail_skew_last_opt(
    ...
    for index in start..end {
        match values.value_at(index) {
            Some(value) if value.is_finite() => valid.push(value),
            Some(_) => return Ok(Some(f64::NAN)),
            None => {}
        }
    }
...
pub fn linear_regression_slope(values: &[f64]) -> Option<f64> {
    ...
    finite_opt(Some((n * sum_xy - sum_x * sum_y) / denom))
}
...
pub fn cmo_last_from_series(values: &(impl F64SeriesView + ?Sized), period: usize) -> Option<f64> {
    ...
    finite_opt(Some(100.0 * (avg_gain - avg_loss) / den))
}
...
pub fn std_pop(values: &[f64]) -> Option<f64> {
    ...
    let var_sum = values
        ...
        .sum::<f64>();
    let value = (var_sum / values.len() as f64).sqrt();
    if value.is_finite() {
        Some(value)
    } else {
        Some(0.0)
