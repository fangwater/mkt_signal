...
impl WsConnector {
    ...
    pub async fn connect_with_local_ip(url: &str, sub_msg: &serde_json::Value, local_ip: &str) -> anyhow::Result<WsConnectionResult> {
        // 如果是默认IP，使用标准连接
        ...
        let host = ws_url.host_str().ok_or_else(|| anyhow::anyhow!("URL has no host"))?;
        ...
        let (mut ws_stream, _resp) = if scheme.eq_ignore_ascii_case("wss") {
            ...
            let host = ws_url.host_str().ok_or_else(|| anyhow::anyhow!("URL has no host"))?;
            let native = NativeTlsConnector::builder().build().with_context(|| "Failed to build native-tls connector")?;
            let connector = TokioTlsConnector::from(native);
            let tls_stream = connector.connect(host, stream).await.with_context(|| "TLS handshake failed")?;
