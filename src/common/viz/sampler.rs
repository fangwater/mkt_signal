...
impl Sampler {
    ...
    pub fn build_snapshot(&mut self) -> Snapshot {
        ...
        for (symbol, entry) in price_entries {
            if let Some(e) = entry {
                prices.push(PriceItem {
                    symbol,
                    mark: Some(e.mark_price).filter(|v| *v != 0.0),
                    index: Some(e.index_price).filter(|v| *v != 0.0),
                    ts: e.update_time,
                });
            } else {
                prices.push(PriceItem {
        ...
        for p in &prices {
            let basis = match (p.mark, p.index) {
                (Some(m), Some(i)) if i.abs() > f64::EPSILON => Some((m - i) / i),
                _ => None,
            };
            factors.push(FactorItem {
                symbol: p.symbol.clone(),
                basis,
                open_threshold: self.thresholds.open,
                close_threshold: self.thresholds.close,
            });
        }
        ...
        for p in &prices {
            let (funding_rate, predicted, next_funding_time, loan_rate_8h) =
                self.state.funding_for(&p.symbol, self.exchange, ts);
            funding.push(FundingItem {
                symbol: p.symbol.clone(),
                funding_rate,
        ...
        let pnl = PnlSummary {
            unrealized: self.state.compute_unrealized_pnl(),
        };
