...
///
/// # 环境变量
/// - `MKT_SIGNAL_NAMESPACE`: 必须存在，用于命名空间隔离（例如: "mkt1", "mkt2"）
///
/// # 示例
/// ```
/// // 环境变量: MKT_SIGNAL_NAMESPACE=mkt1
///
/// // 市场数据（公用）：
...
///
/// # Panics
/// - 如果环境变量 `MKT_SIGNAL_NAMESPACE` 未设置，将 panic
pub fn build_service_name(base_name: &str) -> String {
    ...
    // 读取必须存在的命名空间环境变量
    let namespace = env::var("MKT_SIGNAL_NAMESPACE")
        .expect("环境变量 MKT_SIGNAL_NAMESPACE 未设置！请设置该变量以隔离 IceOryx 服务命名空间");
