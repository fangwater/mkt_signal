...
const OKEX_STATE_PARTIALLY_FILLED: u8 = 3;
const OKEX_STATE_FILLED: u8 = 4;

#[derive(Debug, Parser)]
#[command(
    name = "okex_am_latency_probe",
    about = "Measure OKEX account-monitor order trade and trade-lite latency in microseconds"
)]
struct Args {
    #[arg(long, default_value = "account_pubs/okex_pm")]
    service: String,

    #[arg(long, default_value = "okex_am_latency_probe")]
    node_name: String,

    #[arg(long, default_value = "logs/okex_am_latency_probe.log")]
    output: PathBuf,
...
fn parse_latency_sample(payload: &[u8]) -> Option<LatencySample> {
    ...
    match event_type {
        BasicAccountEventType::OrderUpdate => {
            let msg = OkexOrderMsg::from_bytes(data).ok()?;
            if !is_okex_order_trade_state(msg.state) {
                return None;
            }
            let event_time = if msg.fill_time > 0 {
                msg.fill_time
            } else if msg.update_time > 0 {
                msg.update_time
            } else {
                msg.create_time
            };
            ...
            let event_ts_us = ms_to_us(event_time);
            Some(LatencySample {
                kind: LatencyKind::OrderTrade,
                scope,
                venue: okex_order_venue(&msg),
                event_ts_us,
                local_ts_us,
                ...
                raw_execution_type: format!(
                    "OrderUpdate({})",
                    OkexOrderMsg::inst_type_to_str(msg.inst_type)
                ),
                raw_status: OkexOrderMsg::state_to_str(msg.state).to_string(),
            })
        }
...
fn is_okex_order_trade_state(state: u8) -> bool {
    matches!(state, OKEX_STATE_PARTIALLY_FILLED | OKEX_STATE_FILLED)
}

fn okex_order_venue(msg: &OkexOrderMsg) -> u8 {
    match msg.inst_type {
        2 | 3 => TradingVenue::OkexFutures as u8,
        _ => TradingVenue::OkexMargin as u8,
    }
}
