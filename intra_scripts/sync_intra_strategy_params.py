...
STRATEGY_PARAMS = {
    ...
    "enable_taker_decsion_model_cancel": "true",
    "taker_decsion_model_type": "tree_model",
    "taker_decsion_model_service": "-",
    "taker_decsion_nn_model_zmq_ipc": "ipc:///tmp/predict_rnn_layer.ipc",
    ...
    "taker_decsion_model_score_rolling_mean_window": "3",
...
PARAM_COMMENTS: Dict[str, str] = {
    ...
    "enable_volatility_limit": "是否启用基于 inline vol 阈值的开仓 gate；仅 intra_vol_gate_symbols 列表内 symbol 生效；intra 默认 false",
    "open_volatility_limit": "开仓 vol gate 使用的 rolling 分位数（0-100，默认 70）",
    "vol_gate_compare": "vol gate symbol 的放行方向：lt 表示 vol < threshold；gt 表示 vol > threshold",
    ...
    "enable_taker_decsion_model_cancel": "是否启用基于 taker decision score 的 open order cancel；默认 true，只影响撤单，不影响 keep long/short",
    ...
    "taker_decsion_nn_model_keep_long_score": "nn_model score 高于该固定值时 keep long",
    "taker_decsion_nn_model_keep_short_score": "nn_model score 低于该固定值时 keep short",
    "taker_decsion_nn_model_open_cancel_long_score": "nn_model score 低于该固定值时撤掉 open long",
    "taker_decsion_nn_model_open_cancel_short_score": "nn_model score 高于该固定值时撤掉 open short",
    "taker_decsion_model_score_rolling_mean_window": "taker decision 模型 raw score rolling mean 窗口，正整数；默认 3，1 表示不平滑",
    "taker_decsion_model_keep_long_percentile": "模型高于该配置分位对应 score 阈值时 keep long",
    "taker_decsion_model_keep_short_percentile": "模型低于该配置分位对应 score 阈值时 keep short",
...
PARAM_PRINT_ORDER = [
    ...
    "taker_decsion_model_type",
    "taker_decsion_model_service",
    "taker_decsion_nn_model_zmq_ipc",
    "taker_decsion_nn_model_keep_long_score",
    "taker_decsion_nn_model_keep_short_score",
    "taker_decsion_nn_model_open_cancel_long_score",
    "taker_decsion_nn_model_open_cancel_short_score",
