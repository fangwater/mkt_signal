...
"""
打印 intra per-symbol max_pos_u 覆盖配置（从 Redis 读取，不修改）。
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print intra max_pos_u overrides from Redis")
...
def main() -> int:
    ...
    print(f"🔎 读取 intra max_pos_u 覆盖配置: {key}")
