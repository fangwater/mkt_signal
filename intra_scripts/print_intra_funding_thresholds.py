...
"""
打印 intra（同所期现）funding 阈值配置（从 Redis 读取）。

读取 Redis Hash:
  `intra_funding_thresholds_{open_venue}_{hedge_venue}`

注：intra 是同所现期，funding rate 概念不强（同所没有跨所费率差），但保留 filter 接口
以兼容 evaluate_open_gate 中的 enable_funding_open_filter（实际多用作 premium_rate）。
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print intra funding thresholds from Redis")
...
def main() -> int:
    ...
    key = f"intra_funding_thresholds_{args.open_venue}_{args.hedge_venue}"
    config_key = f"intra_funding_thresholds_config_{args.open_venue}_{args.hedge_venue}"
    rds = redis.Redis(host="127.0.0.1", port=6379, db=0, password=None)

    print("📍 Redis: 127.0.0.1:6379/0")
    print(f"📊 intra funding 阈值: {key}")
    print("-" * 80)
    ...
    if not threshold_order:
        threshold_order = infer_threshold_order_from_data(list(kv.keys()))
        if not threshold_order:
