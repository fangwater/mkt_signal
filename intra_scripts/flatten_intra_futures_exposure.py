#!/usr/bin/env python3
"""Flatten intra (intra-exchange spot/futures arb) net exposure by trading UM futures.
...
Usage:
  # from the intra deploy directory (env.sh already sourced by wrapper script)
  python flatten_intra_futures_exposure.py --suffix trade
  python flatten_intra_futures_exposure.py --suffix trade --min-net-usdt 10 --execute
"""
...
def fetch_snapshot(exchange: str, suffix: str) -> List[ExposureRow]:
    ...
    intra dashboard nginx prefix: /intra/<exchange>-intra-<suffix>/
    """
    url = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Flatten intra (intra-exchange) net exposure (Binance UM futures)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--exchange",
        default=os.environ.get("EXCHANGE", "binance"),
        help="exchange name (intra is intra-exchange; default binance)",
    )
    parser.add_argument(
        "--suffix",
        required=True,
...
def main() -> None:
    ...
    exchange = (args.exchange or "binance").strip().lower()
    ...
    for row in targets:
        ...
        # net_qty > 0 → long exposure → SELL futures to flatten
        # net_qty < 0 → short exposure → BUY futures to flatten
