...
description: Analyze and compare intra spot/futures arbitrage order latency from local uniform_orders parquet snapshots. Use when the user asks to split signal latency by spot versus futures market trigger, measure create_ts to signal_ts internal latency, compare margin and futures NEW acknowledgement latency, map futures hedge latency, or analyze NEW rows that later filled for environments such as bybit-intra-arb01, okex-intra-arb01, gate-intra-arb01, bitget-intra-arb01, or binance-intra-arb01.
---
...
4. For same-exchange spot/futures intra analysis, report spot and futures results in one combined three-line table, not as separate sections.
5. Include a Chinese `含义` column and `p50/p90/p95/p99` columns in milliseconds.
6. For margin/open-leg rows, report:
   - `signal_ts - trigger_mkt_ts` as separate `spot`, `futures`, and `tie` trigger rows
   - `margin.create_ts - signal_ts`
   - `margin.NEW.update_ts - margin.create_ts`
   - `margin.NEW.local_ts - margin.create_ts`
7. For futures hedge rows, report:
   - `futures.create_ts - margin.local_ts`
   - `futures.create_ts - margin.update_ts`
   - `futures.NEW.update_ts - futures.create_ts`
   - `futures.NEW.local_ts - futures.create_ts`
...
9. Show the normal-path sample count and the corresponding total `NEW` count so exclusions and unmatched hedge rows remain visible.
...
Do not use one aggregate `signal_ts - trigger_mkt_ts` row as the primary comparison
when dual-BBO fields are present. Put `spot`, `futures`, and `tie` trigger classes on
separate rows so spot and futures market-data paths can be compared directly. Include
an aggregate only as a secondary reference.

## Internal Signal-To-Create

Use `margin.create_ts - signal_ts` as the margin/open-leg system-internal pre-order
latency. It starts when `trade_signal` generates the signal and ends when `pre_trade`
first publishes the margin new-order request toward `trade_engine`. It includes
signal IPC/queueing, pre-trade checks, request construction, and publish cost, but
excludes exchange and Internet latency.
...
Do not combine a futures hedge row's `create_ts - signal_ts` with the margin/open-leg
metric. On a hedge row, `signal_ts` can represent the margin trigger event rather than
the original `trade_signal` open signal. Report it only as a separately named futures
hedge reaction metric when explicitly needed.
...
Select `status == NEW` rows from `BinanceFutures`. Parse `from_key` as:

```text
<margin_open_client_order_id>|<arb_hedge_reason>|<margin_trigger_update_ts>[:metadata]
```

Extract only the leading integer from the third component. Deduplicate
`BinanceMargin` trigger rows by `(client_order_id, update_ts)`, keep the earliest
`local_ts`, and left-join futures rows on both identifiers. Report unmatched futures
rows explicitly.

Report these Binance metrics:

- `futures.create_ts - margin.local_ts`: taker 触发内部延迟.
- `futures.create_ts - margin.update_ts`: 现货撤单/成交到合约请求创建延迟.
- `futures.NEW.update_ts - futures.create_ts`: 合约挂单延迟.
- `futures.NEW.local_ts - futures.create_ts`: 合约完整回报延迟.

## Combined Table Meanings

- `signal_ts - trigger_mkt_ts`: 按现货/合约触发源拆分的交易所行情事件到信号延迟；不是纯系统内部延迟.
- `signal_ts - mkt_ts`: 旧记录的兼容行情延迟，触发腿未知.
- `margin.create_ts - signal_ts`: 现货开单信号到新订单请求首次 publish 的系统内部延迟.
- `margin.NEW.update_ts - margin.create_ts`: 现货挂单延迟.
- `margin.NEW.local_ts - margin.create_ts`: 现货完整回报延迟.
- `futures.create_ts - margin.local_ts`: taker触发内部延迟.
- `futures.create_ts - margin.update_ts`: 合约挂到距离现货撤单/成交延迟.
- `futures.NEW.update_ts - futures.create_ts`: 合约挂单延迟.
- `futures.NEW.local_ts - futures.create_ts`: 合约完整回报延迟.

Never label a Margin `NEW.update_ts - create_ts` row as 合约挂单延迟. Use the
venue-qualified endpoint names above whenever both spot and futures rows appear.
