...
thread_local! {
    // 序列化缓冲跨 emit 调用复用；单线程 LocalSet、emit 无重入，borrow 安全。
    static EMIT_CONTEXT_BUF: RefCell<BytesMut> = RefCell::new(BytesMut::with_capacity(
...
where
    TCtx: SignalBytes,
{
    EMIT_CONTEXT_BUF.with(|buf| {
        let mut context = buf.borrow_mut();
        let mut sent = 0usize;
        for ctx in contexts {
