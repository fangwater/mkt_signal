...
pub fn build_quote_plan_levels<T, V>(
    venue: V,
    symbol: &str,
    order_amount_u: f64,
...
where
    ...
    V: Into<Venue> + Copy,
{
    build_quote_plan_levels_inner(venue, symbol, order_amount_u, None, specs, table)
}

pub fn build_quote_plan_levels_for_base_qty<T, V>(
    venue: V,
    symbol: &str,
    target_base_qty: f64,
    specs: &[QuotePlanLevelSpec],
...
where
    ...
    V: Into<Venue> + Copy,
{
    build_quote_plan_levels_inner(venue, symbol, 0.0, Some(target_base_qty), specs, table)
}

fn build_quote_plan_levels_inner<T, V>(
    venue: V,
    symbol: &str,
    order_amount_u: f64,
    target_base_qty: Option<f64>,
    specs: &[QuotePlanLevelSpec],
    table: &T,
) -> Result<(f64, f64, Vec<QuotePlanLevel>), String>
where
    ...
    V: Into<Venue> + Copy,
{
    ...
    let trade_symbol = normalize_symbol_for_venue(symbol, venue_qp);
    if trade_symbol.is_empty() {
        return Err(format!("normalized symbol is empty: {}", symbol));
    }
    let symbol_key = min_qty_symbol_key(venue_qp, &trade_symbol);
    ...
    let split_qty = target_base_qty.map(|qty| qty / specs.len() as f64);
    for (idx, spec) in specs.iter().copied().enumerate() {
        ...
        let raw_qty = split_qty.unwrap_or(order_amount_u / aligned_raw_price);
        ...
        let (aligned_qty, aligned_price) =
            align_order_for_venue(venue_qp, &symbol_key, raw_qty, aligned_raw_price, table)?;
