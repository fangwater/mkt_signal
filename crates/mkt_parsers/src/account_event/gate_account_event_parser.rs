...
impl GateAccountEventParser {
    ...
    fn parse_spot_orders_v2<S: AccountEventSink>(
        ...
        for order in result {
            ...
            let Some((execution_type, order_status)) =
                order_codes::gate_spot_event_to_execution_and_status(event, finish_as)
            else {
                warn!(
    ...
    fn parse_futures_orders<S: AccountEventSink>(
        ...
        for order in result {
            ...
            let Some((execution_type, order_status)) =
                order_codes::gate_futures_event_to_execution_and_status(status, finish_as)
            else {
                warn!(
...
fn select_gate_price_by_update_kind(
    ...
    let use_fill_price = order_codes::gate_order_uses_fill_price(event_or_status, finish_as);
...
fn select_gate_futures_price_by_finish_as(
    ...
    let selected = if order_codes::gate_order_uses_fill_price("", finish_as) {
        fill_price
    } else {
        order_price
    };
...
mod tests {
    ...
    fn gate_usdt_asset_detail_net_position_matches_equity() {
        ...
        assert!(
            (balance.wallet - borrow.borrowed - borrow.interest - 102_126.004737962706).abs()
                < 1e-9
        );
    }
