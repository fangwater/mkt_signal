...
use runtime_common::mkt_cfg::{binance_um_ip_whitelist_mode_enabled, WsRouteConfig, WsRouteKind};
...
impl WsEndpointGroup {
    fn new(handles: Vec<WsEndpointHandle>) -> Self {
        Self {
            handles,
            next_handle_idx: 0,
        }
...
mod route_selection_tests {
    ...
    fn candidate(base_available: bool) -> BinanceUmWsRouteCandidate {
        BinanceUmWsRouteCandidate { base_available }
    }
    ...
    fn binance_um_endpoint_group_round_robins_handles() {
        ...
        let mut group = WsEndpointGroup::new(handles);
    ...
    #[test]
    fn rr_route_picks_first_available_from_start() {
        let candidates = [candidate(true), candidate(true), candidate(true)];
        assert_eq!(select_binance_um_ws_route(&candidates, 1), Some(1));
    }

    #[test]
    fn rr_route_skips_unavailable_endpoints() {
        // start=0 但 0 不可用 → 轮到 1。
        let candidates = [candidate(false), candidate(true), candidate(false)];
        assert_eq!(select_binance_um_ws_route(&candidates, 0), Some(1));
    }

    #[test]
    fn rr_route_wraps_around() {
        // start=2 不可用，绕回到 0。
        let candidates = [candidate(true), candidate(false), candidate(false)];
        assert_eq!(select_binance_um_ws_route(&candidates, 2), Some(0));
    }

    #[test]
    fn rr_route_none_when_all_unavailable() {
        let candidates = [candidate(false), candidate(false)];
        assert_eq!(select_binance_um_ws_route(&candidates, 0), None);
    }

    #[test]
    fn rr_route_none_when_empty() {
        let candidates: [BinanceUmWsRouteCandidate; 0] = [];
        assert_eq!(select_binance_um_ws_route(&candidates, 0), None);
    }
}

/// WS 下单端点候选：统一只看 `is_available()`。
/// dispatch 路由下，`is_available()` 已无条件把 tcp_loss 软暂停的端点算作不可用，
/// 于是同一套 RR 选路天然绕开坏连接；RR 路由从不置软暂停，故行为不变。
#[derive(Clone, Copy, Debug)]
struct BinanceUmWsRouteCandidate {
    base_available: bool,
}

/// 统一的 round-robin 选路：从 `start` 起找到第一个可用端点的下标。
...
/// 日志用：把候选可用性拼成 `[0{um=1,selected=1};1{um=0,selected=0}]`。
...
impl TradeEngine {
    ...
    pub async fn run_with_shutdown(
        ...
        info!("WsRoute: route={}", self.ws_route.route.as_str());
        ...
        let ws_endpoints = if use_ltp_backend {
            ...
            let mut um_client_id = 0usize;
            {
                ...
                um_endpoint_groups.push(WsEndpointGroup::new(group_handles));
            }
        ...
        let ws_route_for_req_worker = self.ws_route.clone();
                ...
                if use_ltp_backend_for_req_worker || TradeTypeMapping::is_websocket(msg.req_type) {
                    ...
                    if is_binance_um_new {
                        if let Some(groups) = binance_um_ws_endpoint_groups.as_mut() {
                            ...
                            // 统一 RR 选路：dispatch 下 is_available() 已把 tcp_loss 暂停的端点算作不可用。
                            let candidates: Vec<BinanceUmWsRouteCandidate> = groups
                                .iter()
                                .map(|group| BinanceUmWsRouteCandidate {
                                    base_available: group.is_available(),
                                })
                                .collect();
                            let selected_idx = select_binance_um_ws_route(&candidates, start);
                            ...
                            if let Some(idx) = selected_idx {
                                ...
                                if selected_handle.is_none() {
                                    info!(
                                        "WsRouteDecision: client_order_id={} route={} route_type=group selected_group={} reason=enqueue_unavailable candidates={}",
                                        client_order_id,
                                        ws_route.route.as_str(),
                                        idx,
                                        format_binance_um_route_candidates(&candidates, selected_idx),
                                    );
                                    warn!(
                                ...
                                info!(
                                    "WsRouteDecision: client_order_id={} route={} route_type=group selected_group=NA reason=no_available_endpoint candidates={}",
                                    msg.client_order_id,
                                    ws_route.route.as_str(),
                                    format_binance_um_route_candidates(&candidates, None),
                                );
                                warn!(
                                    "no Binance UM websocket endpoint group available for client_order_id={}",
                                    msg.client_order_id
                                );
                                let body = serde_json::json!({
                                    "transport": "ws",
                                    "state": "error",
                                    "reason": "no Binance UM websocket endpoint group available",
                                    "clientOrderId": msg.client_order_id,
                                })
                                .to_string();
                                let _ = trade_resp_sink_for_req_worker.send(TradeExecOutcome {
                    ...
                    if let Some(ref mut endpoints) = target_endpoints {
                        ...
                        if is_binance_um_new {
                            ...
                            let candidates: Vec<BinanceUmWsRouteCandidate> = endpoints
                                .iter()
                                .map(|endpoint| BinanceUmWsRouteCandidate {
                                    base_available: endpoint.is_available(),
                                })
                                .collect();
                            target_idx = select_binance_um_ws_route(&candidates, start);
                            if target_idx.is_none() {
                                info!(
                                    "WsRouteDecision: client_order_id={} route={} route_type=endpoint selected_endpoint=NA reason=no_available_endpoint candidates={}",
                                    msg.client_order_id,
                                    ws_route.route.as_str(),
                                    format_binance_um_route_candidates(&candidates, None),
                                );
