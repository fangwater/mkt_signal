...
if [[ -z "$PORT" ]]; then
  ...
  PORT="$(default_mm_port "$inferred_suffix")"
fi
...
if [[ -f "$ROOT_DIR/config/manual_mm_signal.yaml" ]]; then
  ...
  awk -v p="$PORT" '
    BEGIN {done=0}
    /^[[:space:]]*port[[:space:]]*:/ {print "port: " p; done=1; next}
    {print}
    END {if (!done) print "port: " p}
  ' "$ROOT_DIR/config/manual_mm_signal.yaml" > "$tmp_cfg"
  rsync -a "$tmp_cfg" "$TARGET_DIR/config/manual_mm_signal.yaml"
  rm -f "$tmp_cfg"
fi
