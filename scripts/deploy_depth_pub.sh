...
usage() {
  cat <<'EOF'
Usage:
  deploy_depth_pub.sh [trade|test] [--dir <path>]

Defaults:
  trade -> $HOME/depth_pub
  test  -> $HOME/depth_pub_test

Examples:
  bash scripts/deploy_depth_pub.sh
  bash scripts/deploy_depth_pub.sh trade
  bash scripts/deploy_depth_pub.sh test
  bash scripts/deploy_depth_pub.sh trade --dir "$HOME/depth_pub"
...
if [[ -z "$TARGET_DIR" ]]; then
  case "$ENV_TYPE" in
    trade) TARGET_DIR="$HOME/depth_pub" ;;
    test) TARGET_DIR="$HOME/depth_pub_test" ;;
  esac
fi
