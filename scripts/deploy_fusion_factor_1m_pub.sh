...
usage() {
  cat <<'USAGE'
Usage:
  deploy_fusion_factor_1m_pub.sh (--exchange <exchange> | --venue <venue>)

Defaults:
  固定部署根目录 -> $HOME/fusion_factor_1m
  目录结构 -> $HOME/fusion_factor_1m/<venue>/

Examples:
  bash scripts/deploy_fusion_factor_1m_pub.sh --exchange binance
  bash scripts/deploy_fusion_factor_1m_pub.sh --exchange okex
...
Notes:
  ...
  - 每个 venue 会生成独立二进制:
      fusion_factor_1m_pub_<venue>
    并额外创建兼容软链:
      fusion_factor_1m_pub -> fusion_factor_1m_pub_<venue>
USAGE
}

TARGET_ROOT="$HOME/fusion_factor_1m"
EXCHANGE=""
VENUE=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
for venue in "${VENUES[@]}"; do
  ...
  if [[ -f "$ROOT_DIR/config/fusion_factor_1m_pub.toml" ]]; then
    rsync -a "$ROOT_DIR/config/fusion_factor_1m_pub.toml" "$TARGET_DIR/config/"
  fi
done
...
echo "[INFO] 启动示例: cd ${TARGET_ROOT%/}/${VENUES[0]} && ./scripts/start_fusion_factor_1m_pub.sh"
