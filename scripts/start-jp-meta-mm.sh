...
CHECK_ONLY=0
STARTUP_WAIT_SECONDS="${MM_START_WAIT_SECONDS:-15}"
STARTUP_SETTLE_SECONDS="${MM_START_SETTLE_SECONDS:-3}"
LOG_LINES="${MM_START_LOG_LINES:-80}"

usage() {
  cat <<'USAGE'
Usage: scripts/start-jp-meta-mm.sh --env-name <name> [options]
...
require_positive_integer MM_START_WAIT_SECONDS "$STARTUP_WAIT_SECONDS"
require_positive_integer MM_START_SETTLE_SECONDS "$STARTUP_SETTLE_SECONDS"
require_positive_integer MM_START_LOG_LINES "$LOG_LINES"
...
for required_command in bash curl pmdaemon npx ps readlink sed grep ss tail sleep awk; do
...
env_tag="${env_basename#${exchange}_mm_}"
...
if [[ ! "$config_server_port" =~ ^[1-9][0-9]{0,4}$ ]] || ((10#$config_server_port > 65535)); then
  echo "[ERROR] invalid PORT in $target/config/mm_config_server.env: $config_server_port" >&2
  exit 1
fi
...
process_names=(
  "mm_viz_${exchange}_${env_tag}"
  "mm_pm_${exchange}_${env_tag}"
  "mm_te_${exchange}_${env_tag}"
  "mm_pt_${exchange}_${env_tag}"
  "mm_am_${exchange}_${env_tag}"
)
start_scripts=(
  "$mm_scripts_dir/start_mm_viz_server.sh"
  "$mm_scripts_dir/start_mm_persist_manager.sh"
...
require_signals_stopped() {
  ...
  for index in "${!signal_labels[@]}"; do
    mapfile -t pids < <(find_exact_pids "${signal_binaries[$index]}")
    if [[ "${#pids[@]}" -ne 0 ]]; then
      echo "[ERROR] ${signal_labels[$index]} is running (pids=${pids[*]}); refusing to start the base MM stack" >&2
      exit 3
    fi
  done
}
...
cd "$target"
echo "[WARN] LIVE start begins: env=$env_basename exchange=$exchange signal_processes=stopped"
start_and_verify_config_server
for index in "${!labels[@]}"; do
