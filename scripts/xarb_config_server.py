...
# -*- coding: utf-8 -*-
"""
XARB 配置服务器（xarb_config_server）
-----------------------------------
提供页面/接口用于查看和编辑 xarb 相关配置：
- symbol lists
...
XARB_SCRIPT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "xarb_scripts"))
PAIR_RE = re.compile(r"^([a-z0-9]+)[-_]([a-z0-9]+)[-_]xarb([_-].*)?$")
...
try:
    import sync_xarb_strategy_params as strategy_defaults
...
try:
    import sync_xarb_funding_rate_thresholds as funding_defaults
...
spread_sync = None
try:
    import sync_xarb_spread_thresholds as spread_sync
...
<body>
  ...
  <main>
    ...
    <section id="symbol-lists" class="panel">
      ...
      <div class="grid-2">
        <div>
          <h3>平仓列表 <span class="hint">xarb_dump_symbols</span></h3>
          <textarea id="sym-dump" class="mono" placeholder="每行一个 symbol"></textarea>
        </div>
        <div>
          <h3>正套建仓 <span class="hint">xarb_fwd_trade_symbols</span></h3>
          <textarea id="sym-fwd" class="mono" placeholder="每行一个 symbol"></textarea>
        </div>
        <div>
          <h3>反套建仓 <span class="hint">xarb_bwd_trade_symbols</span></h3>
          <textarea id="sym-bwd" class="mono" placeholder="每行一个 symbol"></textarea>
        </div>
        <div>
      <div class="hint">说明：xarb 统一按基础 symbol 管理；支持逗号/空格/换行分隔，保存时会做大写和基础归一化。</div>
        </div>
      </div>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XARB 配置服务器 (xarb_config_server)")
...
def normalize_symbol_list_for_xarb(value: Any) -> List[str]:
...
def make_key_suffix(open_venue: str, hedge_venue: str) -> str:
    ...
    if not open_ex or not hedge_ex:
        raise ValueError(f"invalid xarb venues: open={open_venue}, hedge={hedge_venue}")
...
def resolve_venues(
    ...
    if open_venue or hedge_venue:
        ...
        raise ValueError("xarb 需要 open_venue/hedge_venue（或通过默认配置提供）")

    if open_v == hedge_v:
        raise ValueError(f"xarb requires cross venues: open={open_v}, hedge={hedge_v}")
...
def spread_mapping_key(open_venue: str, hedge_venue: str) -> str:
    return f"xarb_spread_thresholds_config_{open_venue}_{hedge_venue}"
...
def write_spread_mapping(rds, open_venue: str, hedge_venue: str, mapping: Dict[str, str]) -> int:
    key = spread_mapping_key(open_venue, hedge_venue)
...
def sync_spread_thresholds(
    ...
    if spread_sync is None:
        raise RuntimeError("sync_xarb_spread_thresholds.py not available")

    rolling_key = f"rolling_metrics_thresholds_{open_venue}_{hedge_venue}"
    write_key = f"xarb_spread_thresholds_{open_venue}_{hedge_venue}"
    ...
    target_symbols = spread_sync.load_symbol_lists(rds, key_suffix)
    if symbol:
    ...
    for target_symbol in target_symbols:
        rolling_key_symbol = spread_sync.normalize_for_rolling(target_symbol)
    ...
    if all_fields:
        ...
        write_spread_mapping(rds, open_venue, hedge_venue, mapping)
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/symbol-lists":
            ...
            data = {
                ...
                "dump_symbols": read_symbol_list(rds, f"xarb_dump_symbols:{key_suffix}"),
                "fwd_trade_symbols": read_symbol_list(rds, f"xarb_fwd_trade_symbols:{key_suffix}"),
                "bwd_trade_symbols": read_symbol_list(rds, f"xarb_bwd_trade_symbols:{key_suffix}"),
            }
        ...
        if parsed.path == "/api/spread-thresholds":
            try:
                ...
                return
            key = spread_mapping_key(open_venue, hedge_venue)
            values = read_spread_mapping(self.server.context.redis_client, open_venue, hedge_venue)
            if not values:
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/symbol-lists":
            ...
            dump_symbols = normalize_symbol_list_for_xarb(raw_dump)
            fwd_symbols = normalize_symbol_list_for_xarb(raw_fwd)
            bwd_symbols = normalize_symbol_list_for_xarb(raw_bwd)
            ...
            try:
                rds.set(
                    f"xarb_dump_symbols:{key_suffix}",
                    json.dumps(dump_symbols, ensure_ascii=False),
                )
                rds.set(
                    f"xarb_fwd_trade_symbols:{key_suffix}",
                    json.dumps(fwd_symbols, ensure_ascii=False),
                )
                rds.set(
                    f"xarb_bwd_trade_symbols:{key_suffix}",
        ...
        if parsed.path == "/api/strategy-params":
            ...
            if not isinstance(values, dict):
                self._send_error(400, "values must be object")
                return
            key = f"xarb_strategy_params_{open_v}_{hedge_v}"
            written = write_hash(self.server.context.redis_client, key, values)
            self._send_json(200, {"key": key, "count": written})
            return
        ...
        if parsed.path == "/api/funding-thresholds":
            ...
            if not isinstance(values, dict):
                self._send_error(400, "values must be object")
                return
            key = f"xarb_funding_rate_thresholds_{open_v}_{hedge_v}"
            written = write_hash(self.server.context.redis_client, key, values)
            self._send_json(200, {"key": key, "count": written})
            return
        ...
        if parsed.path == "/api/spread-thresholds":
            ...
            written = write_spread_mapping(
                self.server.context.redis_client, open_v, hedge_v, mapping
            )
            self._send_json(200, {"key": key, "count": written})
            return
...
def main() -> int:
    ...
    try:
        ...
        return 2
    print(
        f"🚀 xarb_config_server started on http://{args.host}:{args.port} "
