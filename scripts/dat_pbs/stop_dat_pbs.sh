...
VENUE_DIR_REGEX='^([a-z0-9]+-(futures|margin)|binance-coin-futures)$'
...
Behavior:
  ...
  - venue 由当前目录名自动推断。
  - 删除 pmdaemon 进程名: dps_<ex>_<market>（兼容旧名 dat_pbs_<venue>）
...
find_running_pids() {
  ...
  safe_find_running_pids "dat_pbs" "$venue_arg"
}
