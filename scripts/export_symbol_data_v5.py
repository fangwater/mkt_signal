...
def main():
    parser = argparse.ArgumentParser(description="导出指定 symbol 的订单和信号数据 (v5)")
    parser.add_argument(
        "--dir",
    ...
    parser.add_argument(
        "--workers",
        type=int,
        help="--all 模式下并行线程数 (默认: min(4, CPU 核心数))",
    )
    ...
    if args.all:
        ...
        symbol_list = sorted(symbol_keys)
        if not symbol_list:
            print("⚠️ 未找到任何 symbol")
            return
        ...
        max_workers = min(max_workers, len(symbol_list))
        ...
        print(f"并行导出 {len(symbol_list)} 个 symbol, workers={max_workers}")
        errors = []
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for symbol_key in symbol_list:
                ...
                future_map[future] = symbol_key

            for future in concurrent.futures.as_completed(future_map):
                symbol_key = future_map[future]
                try:
                    future.result()
                except Exception as exc:
                    errors.append((symbol_key, exc))

        if errors:
            for symbol_key, exc in errors:
                print(f"❌ {symbol_key} 导出失败: {exc}")
            raise SystemExit(f"{len(errors)} 个 symbol 导出失败")
        return
