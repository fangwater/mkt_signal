...
usage() {
  cat <<'EOF'
用法: scripts/deploy_intra_trade_engine.sh --exchange <binance> [--env-suffix intra-trade] [--env-name binance-intra-trade] [--jobs <n>] [--cargo-target-dir <path>]

说明:
  - 同所期货现货套利专用，单一 exchange
  - 构建 trade_engine 并拷贝到 $HOME/<exchange>-<env_suffix>/
  - 同步辅助脚本: intra_scripts/{start,stop}_intra_trade_engine.sh

示例:
  scripts/deploy_intra_trade_engine.sh --exchange binance
  scripts/deploy_intra_trade_engine.sh --env-name binance-intra-trade
EOF
}
...
echo "[INFO] 启动: cd $TARGET_DIR && ./intra_scripts/start_intra_trade_engine.sh"
echo "[INFO] 停止: cd $TARGET_DIR && ./intra_scripts/stop_intra_trade_engine.sh"
