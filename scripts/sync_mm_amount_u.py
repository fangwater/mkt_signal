...
"""
同步 MM amount_u 覆盖配置到 Redis 并打印。
...
说明:
  - 仅 MM 使用该配置。
  - symbol 会自动规范化为大写字母数字格式（去掉常见分隔符）。
  - value 必须是大于 0 的数字。
  - 该配置用于覆盖 strategy params 中的 default_order_amount。

示例:
  cd ~/binance_mm_beta
  python scripts/sync_mm_amount_u.py --json '{"BTCUSDT":150,"eth-usdt":80}'
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Sync MM amount_u overrides(JSON symbol->amount_u) to Redis"
    )
    p.add_argument(
        "--json",
        default="{}",
        help='JSON mapping, e.g. \'{"BTCUSDT":150,"ETH-USDT":80}\'',
    )
    return p.parse_args()
...
def main() -> int:
    ...
    print(f"🔄 同步 MM amount_u 覆盖配置: {key}")
