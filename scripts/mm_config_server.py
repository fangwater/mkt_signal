...
<body>
  ...
  <main>
    ...
    <section class="panel">
      ...
      <div class="hint">
        `enable_open_cancel` 只控制旧的 return-score MMCancel；`enable_tlen_cancel` 单独控制基于 tlen 的 trigger/query/cancel 链路；`enable_return_score_adjust_hegde=false` 时，MM hedge offset 不再被 return score 调整；前端布尔项使用下拉框编辑。
      </div>
    ...
    function buildParamRows(containerId, defaults, comments, order, values) {
      ...
      keys.forEach((key) => {
        ...
        const useBooleanSelect =
          ...
          ['prediction_mode', 'enable_open_cancel', 'enable_tlen_cancel', 'enable_return_score_adjust_hegde'].includes(key) &&
