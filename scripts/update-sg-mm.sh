...
SSH_HOST="${SG_MM_UPDATE_HOST:-ubuntu@47.131.162.78}"
SSH_KEY="${SG_MM_UPDATE_KEY:-$ROOT_DIR/aws-sg.pem}"
ENV_NAME=""
ALL_ENVS=0

usage() {
  cat <<'USAGE'
Usage: scripts/update-sg-mm.sh (--env-name <name> | --all) [options]
...
Live update order:
  ...
  4. Start and health-check the base stack; keep signal processes stopped
...
if [[ "$ALL_ENVS" -eq 1 ]]; then
  ENV_NAMES=(bybit_mm_beta bybit_mm_alpha)
else
  ENV_NAMES=("$ENV_NAME")
fi
...
for env_name in "${ENV_NAMES[@]}"; do
  ...
  echo "[PHASE] stop remote MM stack and cancel/verify orders"
  "$ROOT_DIR/scripts/stop-sg-mm.sh" \
    --host "$SSH_HOST" \
    --key "$SSH_KEY" \
    --env-name "$env_name"

  echo "[PHASE] publish prebuilt MM files"
  "$ROOT_DIR/scripts/publish-sg-mm.sh" \
    --host "$SSH_HOST" \
    --key "$SSH_KEY" \
    --env-name "$env_name" \
    --skip-build
  ...
  "$ROOT_DIR/scripts/start-sg-mm.sh" \
    --host "$SSH_HOST" \
    --key "$SSH_KEY" \
    --env-name "$env_name"

  echo "[ENV] update complete: host=$SSH_HOST env=$env_name signal_processes_started=false persist_manager=included"
done

echo
echo "[INFO] SG Bybit MM update complete; environments=${#ENV_NAMES[@]}"
