...
usage() {
  cat <<'EOF'
Usage:
  start_pairmm_resample.sh [--namespace <ns>] (--exchange <exchange> | <exchange> | <venue...>)

Examples:
  ./scripts/start_pairmm_resample.sh --exchange binance
  ./scripts/start_pairmm_resample.sh okex
  ./scripts/start_pairmm_resample.sh binance-futures
  ./scripts/start_pairmm_resample.sh binance-futures binance-margin
...
BIN_CANDIDATES=(
  "${SCRIPT_DIR}/pairmm_resample"
  "${SCRIPT_DIR}/target/release/pairmm_resample"
  "${SCRIPT_DIR}/../pairmm_resample"
  "${SCRIPT_DIR}/../target/release/pairmm_resample"
  "${BASE_DIR}/pairmm_resample"
  "${BASE_DIR}/target/release/pairmm_resample"
)
...
if [[ -z "$BIN_PATH" ]]; then
  echo "[ERROR] pairmm_resample binary not found. Build first with: cargo build --release --bin pairmm_resample" >&2
  exit 1
fi

start_one() {
  local venue="$1"
  local name="pairmm_resample_${venue}"
...
echo "Logs: ${PM2[*]} logs --namespace ${NAMESPACE} pairmm_resample_<venue>"
