...
usage() {
  cat <<'EOF'
Usage:
  start_dat_pbs.sh [--root <path>] (--exchange <exchange> | <exchange> | <venue...>)

Examples:
  ./scripts/start_dat_pbs.sh --exchange bybit
  ./scripts/start_dat_pbs.sh bybit
  ./scripts/start_dat_pbs.sh bybit-futures bybit-margin
  ./scripts/start_dat_pbs.sh --root "$HOME/dat_pbs" --venue bybit-futures
...
Notes:
  ...
  - Each venue is started by entering <root>/<venue>/ and invoking ./scripts/start_dat_pbs.sh.
EOF
}
...
for venue in "${VENUES[@]}"; do
  ...
  start_script="$venue_dir/scripts/start_dat_pbs.sh"
  if [[ ! -x "$start_script" ]]; then
    echo "[ERROR] 未找到可执行启动脚本: $start_script" >&2
    echo "[HINT] 先执行: bash ${BASE_DIR}/scripts/deploy_dat_pbs.sh --venue ${venue}" >&2
    exit 1
  fi

  echo "[INFO] 启动 dat_pbs venue=${venue}"
  (
    cd "$venue_dir"
