...
CFG_ENV_FILE="${BASE_DIR}/config/mm_config_server.env"
if [[ -f "$CFG_ENV_FILE" ]]; then
  # shellcheck disable=SC1090
  source "$CFG_ENV_FILE"
fi

APP_SCRIPT="${SCRIPT_DIR}/mm_config_server.py"
if [[ ! -f "$APP_SCRIPT" ]]; then
  echo "[ERROR] 未找到 mm_config_server.py (期望路径: $APP_SCRIPT)" >&2
  exit 1
fi
...
detected_exchange=""
case "$dir_name" in
  *okex*|*OKEX*|*okx*|*OKX*) detected_exchange="okex" ;;
...
EXCHANGE="${EXCHANGE:-${DEFAULT_EXCHANGE:-${detected_exchange:-binance}}}"
...
APP_NAME="${PM2_NAME:-mm_config_server_${dir_tag}}"
...
(
  cd "$BASE_DIR"
  PORT="$PORT" HOST="$HOST" DEFAULT_EXCHANGE="$DEFAULT_EXCHANGE" ENV_NAME="$ENV_NAME" \
