...
The exchange is inferred from env-name. Binance cta targets the
jp-meta-elvpn SSH alias.
USAGE
}
...
if [[ "$ALL_ENVS" -eq 1 ]]; then
  for env_name in "${CTA_ORCHESTRATION_ENVS[@]}"; do
    ...
    "$ROOT_DIR/scripts/stop-cta.sh" "${child_args[@]}"
  done
  exit 0
fi
