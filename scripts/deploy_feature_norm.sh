...
usage() {
  cat <<'USAGE'
Usage:
  deploy_feature_norm.sh --exchange <exchange>

Defaults:
  固定部署根目录 -> $HOME/feature_norm
  目录结构 -> $HOME/feature_norm/<venue>/

Examples:
  bash scripts/deploy_feature_norm.sh --exchange binance
  bash scripts/deploy_feature_norm.sh --exchange okex
...
Notes:
  ...
  - 每个 venue 会生成独立二进制:
      feature_norm_<venue>
    并额外创建兼容软链:
      feature_norm -> feature_norm_<venue>
USAGE
}

TARGET_ROOT="$HOME/feature_norm"
EXCHANGE=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --exchange)
...
for venue in "${VENUES[@]}"; do
  ...
  if [[ -f "$ROOT_DIR/config/feature_norm.yaml" ]]; then
    rsync -a "$ROOT_DIR/config/feature_norm.yaml" "$TARGET_DIR/config/"
  fi
done
...
echo "[INFO] 启动示例: cd ${TARGET_ROOT%/}/${VENUES[0]} && ./scripts/start_feature_norm.sh"
