  ...
  <main>
    ...
    <section id="symbol-lists" class="panel">
      ...
      <div class="grid-2">
        ...
        <div>
          <h3>UniMMR 平仓候选 <span class="hint">{env}:fr_unimmr_close_symbols</span></h3>
...
def sync_spread_thresholds(
    ...
    unimmr_close_keys = [build_symbol_list_key("unimmr_close_symbols", key_suffix)]
    ...
    target_symbols = spread_sync._get_target_symbols(
        rds, dump_keys, fwd_keys, bwd_keys, symbol, unimmr_close_keys
    )
...
class RequestHandler(BaseHTTPRequestHandler):
        ...
        if parsed.path == "/api/symbol-lists":
            ...
            data = {
                ...
                "unimmr_close_symbols": read_symbol_list(
                    rds, build_symbol_list_key("unimmr_close_symbols", key_suffix)
                ),
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/symbol-lists":
            ...
            field_specs = (
                ("dump_symbols", "dump"),
                ("unimmr_close_symbols", "unimmr_close"),
                ("fwd_trade_symbols", "fwd"),
                ("bwd_trade_symbols", "bwd"),
            )
            provided = [
            ...
            normalized_lists: Dict[str, List[str]] = {
                name: normalize_symbol_list_for_exchange(exchange, value)
                for name, value in provided
            }
            skipped_fields = [name for name, _ in field_specs if name not in normalized_lists]
            print(
                "[symbol-lists] env={} exchange={} open={} hedge={} key_suffix={} skipped={}".format(
                    infer_dir_prefix_from_cwd() or "-",
                    exchange,
                    open_v,
                    hedge_v,
                    key_suffix,
            ...
            try:
                for name, symbols in normalized_lists.items():
                    rds.set(
                        build_symbol_list_key(name, key_suffix),
