...
usage() {
  cat <<'EOF'
用法: scripts/deploy_cross_config_server.sh --env-name <open>-<hedge>-cross-<suffix>
                                          [--open-venue <okex-futures>]
                                          [--hedge-venue <binance-futures>]
                                          [--target <path>]
                                          [--bind 0.0.0.0] [--port <default>]
                                          [--nginx-prefix /cross/<env-name>/config]
                                          [--nginx-port 4191]
...
示例:
  scripts/deploy_cross_config_server.sh --env-name okex-binance-cross-trade
  scripts/deploy_cross_config_server.sh --env-name okex-binance-cross-trade --apply-nginx
EOF
}
...
require_cross_env_name() {
  local name="$1"
  if [[ ! "$name" =~ ^[a-z0-9]+-[a-z0-9]+-cross-[a-z0-9][a-z0-9_-]*$ ]]; then
    echo "[ERROR] env-name must match <open>-<hedge>-cross-<suffix> (got: ${name})" >&2
    exit 1
  fi
}
...
upsert_main_nginx_mapping() {
  ...
  begin_marker="# BEGIN managed: cross config ${base_prefix}"
  end_marker="# END managed: cross config ${base_prefix}"
  ...
  awk -v begin="$begin_marker" \
    ...
    in_block && $0 == end {
        in_block = 0;
        print begin;
        print "# cross config server (HTTP)";
...
if [[ -z "$ENV_NAME" ]]; then
  echo "[ERROR] 需要使用 --env-name 指定部署环境名（例如 okex-binance-cross-trade）" >&2
  usage
  exit 1
fi
...
if [[ "$OPEN_VENUE" == "$HEDGE_VENUE" ]]; then
  echo "[ERROR] cross open/hedge venue 不能完全相同：open=${OPEN_VENUE} hedge=${HEDGE_VENUE}" >&2
  exit 1
fi
...
if [[ -z "$NGINX_PREFIX" ]]; then
  NGINX_PREFIX="/cross/${ENV_NAME}/config"
fi
...
FILES=(
  ...
  "cross_scripts/sync_cross_funding_thresholds.py"
  "cross_scripts/sync_cross_spread_thresholds.py"
...
cat <<EOF > "$TARGET_DIR/config/cross_config_server.env"
HOST=${BIND}
PORT=${PORT}
...
echo "[INFO] 已部署 cross_config_server 脚本到 $DEST_SCRIPT_DIR"
echo "[INFO] 默认端口: ${PORT}"
echo "[INFO] 默认 venues: open=${OPEN_VENUE} hedge=${HEDGE_VENUE}"
echo "[INFO] 启动: cd $TARGET_DIR && PORT=${PORT} ./scripts/start_cross_config_server.sh"
echo "[INFO] 停止: cd $TARGET_DIR && ./scripts/stop_cross_config_server.sh"
