...
usage() {
  cat <<'USAGE'
Usage:
  start_kline_factor_pub.sh [--namespace <ns>] (--exchange <exchange> | <exchange> | <venue...>)

Examples:
  ./scripts/start_kline_factor_pub.sh --exchange binance
  ./scripts/start_kline_factor_pub.sh okex
  ./scripts/start_kline_factor_pub.sh binance-futures
  ./scripts/start_kline_factor_pub.sh binance-futures binance-margin
...
BIN_CANDIDATES=(
  "${SCRIPT_DIR}/kline_factor_pub"
  "${SCRIPT_DIR}/target/release/kline_factor_pub"
  "${SCRIPT_DIR}/../kline_factor_pub"
  "${SCRIPT_DIR}/../target/release/kline_factor_pub"
  "${BASE_DIR}/kline_factor_pub"
  "${BASE_DIR}/target/release/kline_factor_pub"
)
...
if [[ -z "$BIN_PATH" ]]; then
  echo "[ERROR] kline_factor_pub binary not found. Build first with: cargo build --release --bin kline_factor_pub" >&2
  exit 1
fi
...
echo "Logs: ${PM2[*]} logs --namespace ${NAMESPACE} kline_factor_pub_<venue>"
