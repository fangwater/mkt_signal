...
if [[ "$ALL_ENVS" -eq 1 ]]; then
  for env_name in "${CTA_ORCHESTRATION_ENVS[@]}"; do
    ...
    "$ROOT_DIR/scripts/start-cta.sh" "${child_args[@]}"
  done
  exit 0
fi
...
if [[ "$(basename "$target")" != "$env_name" || "$env_name" != "${exchange}-cta-"* ]]; then
...
require_trade_signal_stopped() {
  ...
  mapfile -t pids < <(find_exact_pids "$trade_signal_binary")
  if [[ "${#pids[@]}" -ne 0 ]]; then
    echo "[ERROR] trade_signal is already running (pids=${pids[*]}); refusing to start the base cta stack" >&2
    exit 3
  fi
}
...
require_all_stopped() {
  ...
  if [[ "$found" -ne 0 ]]; then
    echo "[ERROR] start requires a fully stopped target; run stop-cta.sh first" >&2
    exit 3
  fi
}
