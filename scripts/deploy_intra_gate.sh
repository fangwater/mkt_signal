...
usage() {
  cat <<'EOF'
用法:
  scripts/deploy_intra_gate.sh --env-suffix <suffix> [--bin]
  scripts/deploy_intra_gate.sh <suffix>

说明:
  - 默认只部署到本机 $HOME/gate-intra-<suffix>/（不启动进程）。
  - 部署 Gate 同所期现 intra 环境：
      open=gate-margin
      hedge=gate-futures
  - 环境目录固定: $HOME/gate-intra-<suffix>
  - 仅部署，不启动任何进程
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --remote)
      echo "[ERROR] --remote 已移除；gate intra 入口现在只部署本机" >&2
      exit 1 ;;
    -h|--help)    usage; exit 0 ;;
    *)
...
case "$ENV_SUFFIX" in
  trade) CONFIG_PORT="18135" ;;
  arb01) CONFIG_PORT="19201" ;;
  arb02) CONFIG_PORT="19202" ;;
  arb03) CONFIG_PORT="19203" ;;
  *)
...
echo "[INFO] Gate intra deploy-only"
...
echo "[INFO] Gate intra 部署完成（未启动进程）"
