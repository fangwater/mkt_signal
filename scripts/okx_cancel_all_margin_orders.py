#!/usr/bin/env python3
"""Query OKX MARGIN open orders and optionally cancel them.

OKX margin open orders share the same endpoint as spot/swap (instType filter).
...
def normalize_margin_inst_id(raw: str) -> str:
    """BTCUSDT / BTC-USDT / BTC-USDT-SWAP → BTC-USDT (margin pair)."""
    ...
    if s.endswith("USDT") and len(s) > 4:
        return f"{s[:-4]}-USDT"
    raise ValueError(f"unrecognized OKX margin symbol: {raw}")
...
def fetch_open_orders(
    ...
    params: Dict[str, Any] = {"instType": "MARGIN", "limit": str(limit)}
    if inst_id:
    ...
    if not ok:
        print(f"Query failed: http={status} code={code} msg={msg} body={body_text}", file=sys.stderr)
        return [], None, False
...
def cancel_orders(
    ...
    for batch in chunked(orders, MAX_BATCH):
        ...
        for item in data:
            ...
            s_msg = item.get("sMsg", "")
            s_tag = "OK" if s_code in ("", "0") else "ERR"
            if s_tag == "ERR":
                failed = True
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Query OKX MARGIN open orders and optionally cancel them",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Optional instId filter (repeatable), e.g. BTC-USDT (also accepts BTCUSDT / BTC-USDT-SWAP)",
    )
...
def main() -> None:
    ...
    if args.inst_id:
        for raw in args.inst_id:
            inst_ids.append(normalize_margin_inst_id(raw))
    else:
    ...
    for inst_id in inst_ids:
        ...
        while True:
            ...
            for order in batch:
                ...
                if ord_id and ord_id not in seen_ids:
                    seen_ids.add(ord_id)
    ...
    print(f"OKX MARGIN open orders count: {len(all_orders)}")
