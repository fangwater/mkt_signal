...
for kind in signals_arb_open signals_arb_hedge signals_arb_cancel signals_arb_close; do
    echo -n "  $kind ... "
    if curl -sf "${API_BASE}/signals/${kind}/export" -o "${OUTPUT_DIR}/${kind}.parquet"; then
...
if curl -sf "${API_BASE}/order_updates/export" -o "${OUTPUT_DIR}/order_updates.parquet"; then
...
if curl -sf "${API_BASE}/trade_updates/export" -o "${OUTPUT_DIR}/trade_updates.parquet"; then
