...
usage() {
  cat <<'USAGE'
Usage: scripts/build-sg-mm-binaries.sh

Builds every release binary used by the SG Bybit MM stack. Artifacts are
written to target/release so publish-sg-mm.sh consumes exactly this build.
USAGE
}
...
echo "[INFO] building SG Bybit MM release binaries"
(
  cd "$ROOT_DIR"
...
echo "[INFO] SG Bybit MM release build complete; binaries=${#required_binaries[@]} persist_manager=included"
