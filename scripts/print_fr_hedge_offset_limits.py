...
"""
打印 FR per-symbol hedge_price_offset_limit 覆盖配置（从 Redis 读取，不修改）。
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print FR hedge_price_offset_limit overrides from Redis")
...
def print_value(rds, combined_key: str, upper_key: str, lower_key: str) -> None:
    print("\n📊 FR hedge_price_offset_limit 覆盖配置:")
    print("=" * 80)
...
def main() -> int:
    ...
    print(f"🔎 读取 FR hedge_price_offset_limit 覆盖配置")
