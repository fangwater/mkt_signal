...
usage() {
  cat <<'USAGE'
Usage:
  scripts/pin_aws_ena_irq_affinity.sh --iface <enp40s0> --cpus <cpu-list> [--stop-irqbalance] [--execute]

Default is dry-run. Add --execute to write /proc/irq/*/smp_affinity_list.

Examples:
  scripts/pin_aws_ena_irq_affinity.sh --iface enp40s0 --cpus 28
  scripts/pin_aws_ena_irq_affinity.sh --iface enp40s0 --cpus 28 --stop-irqbalance --execute
USAGE
}
...
for irq in "${IRQS[@]}"; do
  ...
  if [[ "$EXECUTE" -eq 1 ]]; then
    ...
    echo "[SET] irq=$irq $current -> $updated"
  else
