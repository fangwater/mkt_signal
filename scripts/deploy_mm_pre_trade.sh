...
usage() {
  cat <<'USAGE'
Usage:
  scripts/deploy_mm_pre_trade.sh --exchange <binance|okex|gate|bybit|bitget> --env-suffix <suffix>
...
if [[ "$DO_SCRIPTS" -eq 1 ]]; then
  ...
  for script in mm_process_name.sh process_match_lib.sh sync_mm_risk_params.py print_mm_risk_params.py; do
