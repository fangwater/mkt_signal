...
usage() {
  cat <<'USAGE'
Usage:
  scripts/deploy_mm_pre_trade.sh [trade|test] --exchange <binance|okex|gate|bybit|bitget> [--env-suffix <suffix>] [--env-name <exchange>_mm_<suffix>] [--scripts-only|--bin-only]
...
if [[ "$DO_SCRIPTS" -eq 1 ]]; then
  ...
  for script in start_mm_pre_trade.sh stop_mm_pre_trade.sh; do
