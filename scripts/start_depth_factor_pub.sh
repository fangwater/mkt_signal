...
usage() {
  cat <<'USAGE'
Usage:
  start_depth_factor_pub.sh [--namespace <ns>] (--exchange <exchange> | <exchange> | <venue...>)

Examples:
  ./scripts/start_depth_factor_pub.sh --exchange binance
  ./scripts/start_depth_factor_pub.sh okex
  ./scripts/start_depth_factor_pub.sh binance-futures
  ./scripts/start_depth_factor_pub.sh binance-futures binance-margin
...
BIN_CANDIDATES=(
  "${SCRIPT_DIR}/depth_factor_pub"
  "${SCRIPT_DIR}/target/release/depth_factor_pub"
  "${SCRIPT_DIR}/../depth_factor_pub"
  "${SCRIPT_DIR}/../target/release/depth_factor_pub"
  "${BASE_DIR}/depth_factor_pub"
  "${BASE_DIR}/target/release/depth_factor_pub"
)
...
if [[ -z "$BIN_PATH" ]]; then
  echo "[ERROR] depth_factor_pub binary not found. Build first with: cargo build --release --bin depth_factor_pub" >&2
  exit 1
fi
...
echo "Logs: ${PM2[*]} logs --namespace ${NAMESPACE} depth_factor_pub_<venue>"
