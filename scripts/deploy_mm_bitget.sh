...
usage() {
  cat <<'EOF'
用法:
  scripts/deploy_mm_bitget.sh --env-suffix <suffix> [--bin|--runtime-only]
  scripts/deploy_mm_bitget.sh <suffix>
...
case "$ENV_SUFFIX" in
  ...
  alpha)
    ...
    ;;
  *)
    echo "[ERROR] bitget MM 仅支持 suffix: alpha|beta（收到: ${ENV_SUFFIX}）" >&2
    exit 1
    ;;
esac

ENV_NAME="bitget_mm_${ENV_SUFFIX}"

echo "[INFO] BITGET MM deploy-only"
...
echo "[INFO] BITGET MM 部署完成（远端 ${FR_DEPLOY_HOST}，未启动进程）"
