...
APP_SCRIPT="${SCRIPT_DIR}/cta_special_dashboard.py"
...
BIND="${CTA_SPECIAL_DASHBOARD_BIND:-127.0.0.1}"
PORT="${CTA_SPECIAL_DASHBOARD_PORT:-10192}"
cfg_file="$(mktemp)"
trap 'rm -f "$cfg_file"' EXIT
printf '{"apps":[{"name":"%s","script":"%s","args":["%s","--bind","%s","--port","%s","--config","config/cta_special.json","--status","run/cta_special_status.json","--execution-status","run/cta_special_execution_status.json","--index","web/cta_special/index.html"],"cwd":"%s"}]}\n' \
  "$PROC_NAME" "$PYTHON_BIN" "$APP_SCRIPT" "$BIND" "$PORT" "$BASE_DIR" >"$cfg_file"
PMDAEMON_NAME="$PROC_NAME" "${SCRIPT_DIR}/stop_cta_special_dashboard.sh"
