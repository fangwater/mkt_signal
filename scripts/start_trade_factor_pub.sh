...
usage() {
  cat <<'USAGE'
Usage:
  start_trade_factor_pub.sh [--namespace <ns>] (--exchange <exchange> | <exchange> | <venue...>)

Examples:
  ./scripts/start_trade_factor_pub.sh --exchange binance
  ./scripts/start_trade_factor_pub.sh okex
  ./scripts/start_trade_factor_pub.sh binance-futures
  ./scripts/start_trade_factor_pub.sh binance-futures binance-margin
...
BIN_CANDIDATES=(
  "${SCRIPT_DIR}/trade_factor_pub"
  "${SCRIPT_DIR}/target/release/trade_factor_pub"
  "${SCRIPT_DIR}/../trade_factor_pub"
  "${SCRIPT_DIR}/../target/release/trade_factor_pub"
  "${BASE_DIR}/trade_factor_pub"
  "${BASE_DIR}/target/release/trade_factor_pub"
)
...
if [[ -z "$BIN_PATH" ]]; then
  echo "[ERROR] trade_factor_pub binary not found. Build first with: cargo build --release --bin trade_factor_pub" >&2
  exit 1
fi

start_one() {
  local venue="$1"
  local name="trade_factor_pub_${venue}"
...
echo "Logs: ${PM2[*]} logs --namespace ${NAMESPACE} trade_factor_pub_<venue>"
