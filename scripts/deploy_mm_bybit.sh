#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

usage() {
  cat <<'EOF'
用法:
  scripts/deploy_mm_bybit.sh --env-suffix <suffix> [--bin|--runtime-only]
  scripts/deploy_mm_bybit.sh <suffix>

说明:
  - 只部署，不启动任何进程。
  - 目前支持固定 suffix:
      1) beta: 使用基础端口
      2) alpha: 在基础端口上 +1
  - 固定端口（bybit mm）:
      config_server = 18141
      viz_server = 10241
      trade_signal = deploy only, no dedicated HTTP port
    alpha 环境分别使用 18142 / 10242。
  - --bin: 仅替换二进制（不改脚本/配置/nginx/env.sh）。
  - --runtime-only: 仅替换二进制和脚本（不改配置/nginx/env.sh）。
EOF
}

if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  usage
  exit 0
fi

ENV_SUFFIX=""
BIN_MODE="0"
RUNTIME_ONLY="0"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --env-suffix)
      ENV_SUFFIX="${2:-}"
      shift 2
      ;;
    --bin)
      if [[ "$RUNTIME_ONLY" == "1" ]]; then
        echo "[ERROR] --bin 与 --runtime-only 互斥" >&2
        exit 1
      fi
      BIN_MODE="1"
      shift
      ;;
    --runtime-only)
      if [[ "$BIN_MODE" == "1" ]]; then
        echo "[ERROR] --bin 与 --runtime-only 互斥" >&2
        exit 1
      fi
      RUNTIME_ONLY="1"
      shift
      ;;
    -h|--help)
      usage
      exit 0
      ;;
    *)
      if [[ -z "$ENV_SUFFIX" ]]; then
        ENV_SUFFIX="$1"
        shift
      else
        echo "[ERROR] 未知参数: $1" >&2
        usage
        exit 1
      fi
      ;;
  esac
done

if [[ -z "$ENV_SUFFIX" ]]; then
  echo "[ERROR] 需要传入 env suffix（alpha|beta）" >&2
  usage
  exit 1
fi

ENV_SUFFIX="$(echo "$ENV_SUFFIX" | tr 'A-Z' 'a-z')"

case "$ENV_SUFFIX" in
  beta)
    CONFIG_PORT="18141"
    VIZ_PORT="10241"
    ;;
  alpha)
    CONFIG_PORT="18142"
    VIZ_PORT="10242"
    ;;
  *)
    echo "[ERROR] bybit MM 仅支持 suffix: alpha|beta（收到: ${ENV_SUFFIX}）" >&2
    exit 1
    ;;
esac

ENV_NAME="bybit_mm_${ENV_SUFFIX}"

echo "[INFO] BYBIT MM deploy-only"
echo "[INFO] env_name=${ENV_NAME}"
echo "[INFO] config_port=${CONFIG_PORT}, viz_port=${VIZ_PORT}"
echo "[INFO] 不会执行 start 命令"
if [[ "$BIN_MODE" == "1" ]]; then
  echo "[INFO] mode=bin (仅替换二进制)"
elif [[ "$RUNTIME_ONLY" == "1" ]]; then
  echo "[INFO] mode=runtime-only (仅替换二进制和脚本)"
fi

if [[ ( "$BIN_MODE" == "1" || "$RUNTIME_ONLY" == "1" ) && ! -d "$HOME/$ENV_NAME" ]]; then
  echo "[ERROR] 仅替换模式要求环境目录已存在: $HOME/$ENV_NAME" >&2
  exit 1
fi

TARGET_DIR="$HOME/$ENV_NAME"

write_env_template_if_missing() {
  local target_dir="$1"
  local env_name="$2"
  local env_file="${target_dir}/env.sh"
  if [[ -f "$env_file" ]]; then
    echo "[INFO] env template exists, keep as-is: $env_file"
    return 0
  fi

  cat >"$env_file" <<EOF
#!/usr/bin/env bash

export IPC_NAMESPACE="${env_name}"

export BYBIT_API_KEY=""
export BYBIT_API_SECRET=""

export RUST_LOG="info"
EOF
  chmod 600 "$env_file"
  echo "[INFO] wrote env template: $env_file"
}

run_deploy() {
  local cmd=("$@")
  echo "[RUN] ${cmd[*]}"
  local output
  local status
  set +e
  output="$("${cmd[@]}" 2>&1)"
  status=$?
  set -e

  if [[ -n "$output" ]]; then
    echo "$output"
  fi

  if [[ "$status" -eq 0 ]]; then
    return 0
  fi

  if echo "$output" | grep -Eiq "text file busy|text busy|etxtbsy"; then
    echo "[WARN] 检测到 Text file busy，跳过该步骤并继续后续部署"
    return 0
  fi

  echo "[ERROR] 命令失败，停止部署: ${cmd[*]}" >&2
  return "$status"
}

cd "$ROOT_DIR"

if [[ "$BIN_MODE" == "1" ]]; then
  run_deploy bash scripts/deploy_mm_account_monitor.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --bin-only

  run_deploy bash scripts/deploy_mm_trade_engine.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --bin-only

  run_deploy bash scripts/deploy_mm_signal.sh \
    --exchange bybit \
    --env-name "$ENV_NAME" \
    --bin-only

  run_deploy bash scripts/deploy_mm_viz_server.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --port "$VIZ_PORT" \
    --bin-only

  run_deploy bash scripts/deploy_mm_persist_manager.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --bin-only

  run_deploy bash scripts/deploy_mm_pre_trade.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --bin-only
elif [[ "$RUNTIME_ONLY" == "1" ]]; then
  run_deploy bash scripts/deploy_mm_config_server.sh \
    --env-name "$ENV_NAME" \
    --exchange bybit \
    --scripts-only

  run_deploy bash scripts/deploy_mm_account_monitor.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"

  run_deploy bash scripts/deploy_mm_trade_engine.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --runtime-only

  run_deploy bash scripts/deploy_mm_signal.sh \
    --exchange bybit \
    --env-name "$ENV_NAME"

  run_deploy bash scripts/deploy_mm_viz_server.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --port "$VIZ_PORT" \
    --runtime-only

  run_deploy bash scripts/deploy_mm_persist_manager.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"

  run_deploy bash scripts/deploy_mm_pre_trade.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"
else
  run_deploy bash scripts/deploy_mm_config_server.sh \
    --env-name "$ENV_NAME" \
    --exchange bybit \
    --port "$CONFIG_PORT" \
    --skip-nginx-apply

  run_deploy bash scripts/deploy_mm_account_monitor.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"

  run_deploy bash scripts/deploy_mm_trade_engine.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"

  run_deploy bash scripts/deploy_mm_signal.sh \
    --exchange bybit \
    --env-name "$ENV_NAME"

  run_deploy bash scripts/deploy_mm_viz_server.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX" \
    --port "$VIZ_PORT"

  run_deploy bash scripts/deploy_mm_persist_manager.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"

  run_deploy bash scripts/deploy_mm_pre_trade.sh \
    --exchange bybit \
    --env-suffix "$ENV_SUFFIX"
fi

if [[ "$BIN_MODE" != "1" && "$RUNTIME_ONLY" != "1" ]]; then
  write_env_template_if_missing "$TARGET_DIR" "$ENV_NAME"
fi

echo "[INFO] BYBIT MM 部署完成（仅 deploy，不含 start）"
if [[ "$BIN_MODE" != "1" && "$RUNTIME_ONLY" != "1" ]]; then
  echo "[INFO] nginx mapping 已写入，但未自动 apply"
  echo "[INFO] To apply nginx (port 4191):"
  echo "       cd ${ROOT_DIR} && PORT=4191 MAPPING_FILE=\$HOME/nginx_locations.txt ./scripts/setup_nginx_4191.sh"
  echo "[INFO] env template: ${TARGET_DIR}/env.sh"
fi
