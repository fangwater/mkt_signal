...
"""
打印 FR per-symbol 单手量(amount_u USDT) 覆盖配置（从 Redis 读取，不修改）。
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print FR amount_u overrides from Redis")
...
def main() -> int:
    ...
    print(f"🔎 读取 FR amount_u 覆盖配置: {key}")
