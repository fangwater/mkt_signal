...
usage() {
  cat <<'USAGE'
Usage: scripts/publish-cta.sh (--env-name <name> | --all) [options]

Options:
  --key <identity>     Optional Bybit/SG SSH identity override
  --env-name <name>   One supported cta environment
  --all                Publish every supported cta environment
...
Supported environments:
  binance-cta-rx01 -> jp-meta-elvpn
...
if [[ "$ALL_ENVS" -eq 1 ]]; then
  ...
  for env_name in "${CTA_ORCHESTRATION_ENVS[@]}"; do
    ...
    "$ROOT_DIR/scripts/publish-cta.sh" "${child_args[@]}"
  done
  exit 0
fi
...
LOCAL_RELATIVE=(
  ...
  "scripts/start_cta_config_server.sh"
  "scripts/stop_cta_config_server.sh"
...
UPLOAD_NAMES=(
  ...
  "start_cta_config_server.sh"
  "stop_cta_config_server.sh"
...
case "$INTRA_EXCHANGE" in
  binance)
    ...
    ;;
  *)
    echo "[ERROR] unsupported exchange in publish manifest: $INTRA_EXCHANGE" >&2
    exit 1
    ;;
esac
...
REMOTE_STAGE="$("${SSH[@]}" "$INTRA_SSH_HOST" "mktemp -d '$REMOTE_DIR/.publish-cta.XXXXXX'")"
case "$REMOTE_STAGE" in
  "$REMOTE_DIR"/.publish-cta.*) ;;
