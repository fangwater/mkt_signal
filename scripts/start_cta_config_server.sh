...
APP_SCRIPT="${SCRIPT_DIR}/cta_config_server.py"
if [[ ! -f "$APP_SCRIPT" ]]; then
  echo "[ERROR] 未找到 cta_config_server.py (期望路径: $APP_SCRIPT)"
  exit 1
fi
...
EXCHANGE=""
if [[ "$dir_lc" =~ ^([a-z0-9]+)[-_]cta[-_][a-z0-9][a-z0-9_-]*$ ]]; then
...
if [[ -z "$EXCHANGE" ]]; then
  echo "[ERROR] 无法确定 exchange (dir=$dir_name)；cta env 目录需为 <exchange>-cta-<tag>" >&2
  exit 1
fi
...
HOST="${HOST:-0.0.0.0}"
PORT="${PORT:-19174}"
APP_NAME="${PM2_NAME:-cta_config_server_${dir_tag}}"
...
echo "[INFO] 启动 cta_config_server (exchange=${EXCHANGE}, port=${PORT}, namespace=${NAMESPACE})"
