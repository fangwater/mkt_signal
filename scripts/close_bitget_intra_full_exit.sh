...
DEFAULT_ENV_NAME="bitget-intra-arb01"

usage() {
  cat <<EOF
用法:
  scripts/close_bitget_intra_full_exit.sh [--execute] [--symbol BTC]
  scripts/close_bitget_intra_full_exit.sh --full-exit [--execute]

说明:
  - Bitget 同所期现盘便捷入口，默认对准 ${DEFAULT_ENV_NAME}
