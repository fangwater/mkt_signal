...
CFG_ENV_FILE="${BASE_DIR}/config/xarb_config_server.env"
if [[ -f "$CFG_ENV_FILE" ]]; then
  # shellcheck disable=SC1090
  source "$CFG_ENV_FILE"
fi

APP_SCRIPT="${SCRIPT_DIR}/xarb_config_server.py"
if [[ ! -f "$APP_SCRIPT" ]]; then
  echo "[ERROR] 未找到 xarb_config_server.py (期望路径: $APP_SCRIPT)"
  exit 1
fi
...
HOST="${HOST:-0.0.0.0}"
APP_NAME="${PM2_NAME:-xarb_config_server_${dir_tag}}"
...
echo "[INFO] 启动 xarb_config_server (open=${DEFAULT_OPEN_VENUE:-"-"}, hedge=${DEFAULT_HEDGE_VENUE:-"-"}, port=${PORT}, namespace=${NAMESPACE})"
