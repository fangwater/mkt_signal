...
usage() {
  cat <<'EOF'
Usage:
  start_kline_pub.sh [--namespace <ns>] (--exchange <exchange> | <exchange> | <venue...>)

Examples:
  ./scripts/start_kline_pub.sh --exchange binance
  ./scripts/start_kline_pub.sh okex
  ./scripts/start_kline_pub.sh binance-futures
  ./scripts/start_kline_pub.sh binance-futures binance-margin
...
BIN_CANDIDATES=(
  "${SCRIPT_DIR}/kline_pub"
  "${SCRIPT_DIR}/target/release/kline_pub"
  "${SCRIPT_DIR}/../kline_pub"
  "${SCRIPT_DIR}/../target/release/kline_pub"
  "${BASE_DIR}/kline_pub"
  "${BASE_DIR}/target/release/kline_pub"
)
...
if [[ -z "$BIN_PATH" ]]; then
  echo "[ERROR] kline_pub binary not found. Build first with: cargo build --release --bin kline_pub" >&2
  exit 1
fi

start_one() {
  local venue="$1"
  local name="kline_pub_${venue}"
...
echo "Logs: ${PM2[*]} logs --namespace ${NAMESPACE} kline_pub_<venue>"
