...
usage() {
  cat <<'EOF'
用法: scripts/deploy_xarb_rolling_metrics.sh [--open-venue <okex-futures>] [--hedge-venue <binance-futures>] [--env-suffix xarb-trade] [--env-name okex-binance-xarb-trade]

说明:
  - 构建 rolling_metrics 并拷贝到 $HOME/<open>-<hedge>-<env_suffix>/ 目录（默认 env_suffix=xarb-trade）。
  - 跨所套利目录名约定: <open>-<hedge>-xarb-trade（例如 okex-binance-xarb-trade）。
  - xarb 固定 futures 资产类型：open/hedge 两侧都会被设置为 <exchange>-futures。
  - 同步辅助脚本: xarb_scripts/print_xarb_rolling_metrics_thresholds.py, xarb_scripts/print_xarb_rolling_metrics_params.py,
    xarb_scripts/sync_xarb_rolling_metrics_params.py, xarb_scripts/start_xarb_rolling_metrics.sh, xarb_scripts/stop_xarb_rolling_metrics.sh, docs/rolling_metrics.md
...
示例:
  ...
  scripts/deploy_xarb_rolling_metrics.sh --env-name okex-binance-xarb-trade
EOF
}
...
EXTRA_FILES=(
  "xarb_scripts/print_xarb_rolling_metrics_thresholds.py"
  "xarb_scripts/print_xarb_rolling_metrics_params.py"
  "xarb_scripts/sync_xarb_rolling_metrics_params.py"
  "xarb_scripts/start_xarb_rolling_metrics.sh"
  "xarb_scripts/stop_xarb_rolling_metrics.sh"
