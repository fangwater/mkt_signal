#!/usr/bin/env python3
"""Flatten xarb futures-spot arb net exposure by trading UM futures.
...
Usage:
  # from the xarb deploy directory (env.sh already sourced by wrapper script)
  python flatten_xarb_futures_exposure.py --suffix trade01
  python flatten_xarb_futures_exposure.py --suffix trade01 --min-net-usdt 10 --execute
"""
...
def fetch_snapshot(suffix: str) -> List[ExposureRow]:
    ...
    url = (
        ...
        f"/xarb/binance-binance-xarb-{suffix}/snapshot"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Flatten xarb futures-spot arb net exposure (Binance UM futures)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--suffix",
        required=True,
        help="env suffix, e.g. 'trade01' → reads from binance-binance-xarb-trade01",
    )
    parser.add_argument(
        "--min-net-usdt",
        type=float,
