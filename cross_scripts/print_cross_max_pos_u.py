...
"""
打印 cross per-symbol max_pos_u 覆盖配置（从 Redis 读取，不修改）。
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print cross max_pos_u overrides from Redis")
...
def main() -> int:
    ...
    print(f"🔎 读取 cross max_pos_u 覆盖配置: {key}")
