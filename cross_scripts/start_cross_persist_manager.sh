...
usage() {
  cat <<'USAGE'
用法: cross_scripts/start_cross_persist_manager.sh
...
说明:
  ...
  - 会以 pmdaemon 启动 1 个进程：
      跨所: cross_pm_<open>_<hedge>_<env>
      同所: cross_pm_<exchange>_<env>
USAGE
}
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  echo "[ERROR] 请先生成并配置：scripts/deploy_setup_env_cross.sh --env-name $(basename "${BASE_DIR}") --open-venue <...> --hedge-venue <...>"
  exit 1
fi
...
if [[ -z "$OPEN_EXCHANGE" || -z "$HEDGE_EXCHANGE" ]]; then
  echo "[ERROR] 无法从目录名推断 open/hedge (dir=$dir_name)，期望 <open>-<hedge>-cross-..."
  exit 1
fi

if [[ "$OPEN_EXCHANGE" == "$HEDGE_EXCHANGE" ]]; then
  DEFAULT_PROC_NAME="cross_pm_${OPEN_EXCHANGE}_${ENV_TAG}"
else
  DEFAULT_PROC_NAME="cross_pm_${OPEN_EXCHANGE}_${HEDGE_EXCHANGE}_${ENV_TAG}"
fi
...
STOP_SCRIPT="${SCRIPT_DIR}/stop_cross_persist_manager.sh"
