...
usage() {
  cat <<'EOF'
用法: cross_scripts/start_cross_viz_server.sh [--cfg config/viz.toml]
...
示例:
  cd $HOME/okex-binance-cross-trade
...
PROC_NAME="cross_viz_${OPEN_EXCHANGE}_${HEDGE_EXCHANGE}_${ENV_TAG}"
if [[ "$OPEN_EXCHANGE" == "$HEDGE_EXCHANGE" ]]; then
  PROC_NAME="cross_viz_${OPEN_EXCHANGE}_${ENV_TAG}"
fi
RUST_LOG="${RUST_LOG:-info}"

if [[ ! -f "$BASE_DIR/$CFG_PATH" ]]; then
  echo "[ERROR] viz config not found: $BASE_DIR/$CFG_PATH"
  echo "[ERROR] 建议先部署：scripts/deploy_cross_viz_server.sh ..."
  exit 1
fi
...
STOP_SCRIPT="${SCRIPT_DIR}/stop_cross_viz_server.sh"
