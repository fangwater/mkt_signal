...
"""
同步 cross（跨所）per-symbol max_pos_u(USDT) 覆盖到 Redis 并打印。
...
示例:
  ...
  python cross_scripts/sync_cross_max_pos_u.py --json '{"BTCUSDT":5000,"eth-usdt":2000}'
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Sync cross max_pos_u overrides to Redis")
    p.add_argument("--open-venue", help="open 侧 venue")
    p.add_argument("--hedge-venue", help="hedge 侧 venue")
...
def main() -> int:
    ...
    print(f"🔄 同步 cross max_pos_u 覆盖配置: {key}")
