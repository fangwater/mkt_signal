...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print cross funding rate thresholds from Redis")
...
def main() -> int:
    ...
    key = f"cross_funding_rate_thresholds_{args.open_venue}_{args.hedge_venue}"
    rds = redis.Redis(host="127.0.0.1", port=6379, db=0, password=None)

    data = rds.hgetall(key)
    print("📍 Redis: 127.0.0.1:6379/0")
    print(f"📊 cross 资金费率阈值: {key}")
    print("-" * 80)
    if not data:
