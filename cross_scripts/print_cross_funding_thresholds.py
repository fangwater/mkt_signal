...
"""
打印 cross funding 阈值配置（从 Redis 读取）。

读取 Redis Hash:
  `cross_funding_thresholds_{open_venue}_{hedge_venue}`
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Print cross funding thresholds from Redis")
...
def main() -> int:
    ...
    key = f"cross_funding_thresholds_{args.open_venue}_{args.hedge_venue}"
    config_key = f"cross_funding_thresholds_config_{args.open_venue}_{args.hedge_venue}"
    rds = redis.Redis(host="127.0.0.1", port=6379, db=0, password=None)

    print("📍 Redis: 127.0.0.1:6379/0")
    print(f"📊 cross funding 阈值: {key}")
    print("-" * 80)
